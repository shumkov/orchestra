...
function collectToolReceipts(branch) {
  ...
  for (const row of branch) {
    ...
    if (row.type === 'user' && Array.isArray(row.message?.content)) {
      for (const block of row.message.content) {
        ...
        toolResults.set(block.tool_use_id, { block, receipt: parseToolResult(block) });
      }
    }
  }
  ...
  for (const toolUse of toolUses.values()) {
    const wrapped = toolResults.get(toolUse.id);
    if (!wrapped || !wrapped.receipt) return { ok: false, reason: 'delivery-result-missing' };
    const receipt = wrapped.receipt;
    ...
    if (receipt.ok === true) {
      ...
      delivered.push({ toolUse, receipt });
    } else if (receipt.ok === false) {
      failuresByAttempt.set(receipt.attempt_id, { toolUse, receipt });
    } else {
  ...
  for (const [attemptId, item] of failuresByAttempt) {
    ...
    unresolvedAttempts.push({
      ...
      toolName: item.toolUse.toolName,
...
async function correlateWorkflowCompletionSnapshot({
  ...
  return result('eligible', {
    eligible: true,
    workflowToolIdHash: crypto.createHash('sha256').update(workflowToolId, 'utf8').digest('hex'),
