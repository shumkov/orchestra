...
class Process extends EventEmitter {
  ...
  async retireForCleanRestart({ getDeliveryEvidence } = {}) {
    if (typeof getDeliveryEvidence === 'function') {
      await getDeliveryEvidence(this.sessionKey, null);
    }
    await this.kill('clean-restart');
    if (!this.closed) {
      throw Object.assign(
        new Error('clean restart retirement did not close the process'),
  ...
  async steerTurn(_text, _opts) {
    throw new UnsupportedOperationError('steerTurn', this.backend);
  }
  ...
  async selectModelSettings(_settings) {
    throw new UnsupportedOperationError('selectModelSettings', this.backend);
  }
  ...
  /**
   * Fence this exact process after an external durability failure. Backends
   * without a durable mutation ledger cannot provide this containment.
   *
   * @returns {boolean} true when the process entered a durable work fence
   */
  blockDurability(_error) {
    return false;
  }
  ...
  /**
   * Does this process have delivery work whose settlement can still emit or
   * suppress user-visible output? ProcessManager treats it as an eviction pin.
   * Backends without deferred delivery decisions return false.
   */
