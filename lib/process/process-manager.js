...
const QUALIFICATION_PROCESS_STATES = new Set([
...
const QUALIFICATION_REASONS = new Set([
...
function qualificationDigest(generationId) {
  return typeof generationId === 'string'
    ? hash('sha256', generationId, 'hex')
    : null;
}

function qualificationResultFor(proc, reason, fence = {}) {
  const observedAtMs = Object.hasOwn(fence, 'observedAtMs')
  ...
  return Object.freeze({
    outcome: 'mismatch',
    reason: QUALIFICATION_REASONS.has(reason) ? reason : 'inspection-failed',
    generationDigest: Object.hasOwn(fence, 'generationDigest')
      ? fence.generationDigest
      : qualificationDigest(proc?.generationId),
    activityEpoch: Object.hasOwn(fence, 'activityEpoch')
      ? fence.activityEpoch
      : (Number.isSafeInteger(proc?.activityEpoch) ? proc.activityEpoch : null),
    processState: QUALIFICATION_PROCESS_STATES.has(proc?.state)
      ? proc.state
      : 'unknown',
    activeTurnCount: proc?.current ? 1 : 0,
    pendingDeliveryCount: typeof proc?.hasPendingDeliveryWork === 'function'
      && proc.hasPendingDeliveryWork() ? 1 : 0,
    backgroundOwnerCount: typeof proc?.hasActiveBackgroundWork === 'function'
      && proc.hasActiveBackgroundWork() ? 1 : 0,
...
function normalizeQualificationExpectation(value, { requireFence = false } = {}) {
  if (value === undefined && !requireFence) {
    return { valid: true, expectedGenerationDigest: null, expectedActivityEpoch: null };
  }
  if (
    value === null
    || typeof value !== 'object'
  ...
  if (
    keys.some((key) => (
      typeof key !== 'string'
      || !['expectedGenerationDigest', 'expectedActivityEpoch'].includes(key)
    ))
    || (requireFence && keys.length !== 2)
    || (!requireFence && keys.length !== 0 && keys.length !== 2)
  ) {
    return { valid: false };
  }
  const expectedGenerationDigest = value.expectedGenerationDigest;
  const expectedActivityEpoch = value.expectedActivityEpoch;
  if (
    keys.length === 2
    && (
      !/^[a-f0-9]{64}$/.test(expectedGenerationDigest)
      || !Number.isSafeInteger(expectedActivityEpoch)
  ...
  return {
    valid: true,
    expectedGenerationDigest: keys.length === 2 ? expectedGenerationDigest : null,
    expectedActivityEpoch: keys.length === 2 ? expectedActivityEpoch : null,
  };
}

function normalizeQualificationResult(proc, result, expectedFence = {}) {
  const expectedKeys = [
    'outcome',
    'reason',
    'generationDigest',
  ...
  if (
    ...
    || !['qualified', 'mismatch'].includes(result.outcome)
    || !QUALIFICATION_REASONS.has(result.reason)
    || !/^[a-f0-9]{64}$/.test(result.generationDigest)
    || !Number.isSafeInteger(result.activityEpoch)
    || result.activityEpoch < 0
    || !QUALIFICATION_PROCESS_STATES.has(result.processState)
    || ![0, 1].includes(result.activeTurnCount)
    || ![0, 1].includes(result.pendingDeliveryCount)
    || ![0, 1].includes(result.backgroundOwnerCount)
    || !Number.isSafeInteger(result.backgroundTerminalCount)
    || result.backgroundTerminalCount < 0
    || result.backgroundTerminalCount > 1_600
    || typeof result.backgroundTerminalRegistryComplete !== 'boolean'
    || !Number.isSafeInteger(result.observedAtMs)
    || result.observedAtMs < 0
  ) {
    return qualificationResultFor(proc, 'inspection-failed');
  }
  if (
    result.outcome === 'qualified'
    && (
      result.reason !== 'eligible'
      || result.generationDigest !== expectedFence.generationDigest
      || result.activityEpoch !== expectedFence.activityEpoch
      || result.processState !== 'Idle'
      || result.activeTurnCount !== 0
      || result.pendingDeliveryCount !== 0
      || result.backgroundOwnerCount !== 0
      || result.backgroundTerminalCount !== 0
      || result.backgroundTerminalRegistryComplete !== true
    )
  ) {
    return qualificationResultFor(proc, 'incoherent', {
      ...expectedFence,
  ...
  return Object.freeze({
    ...
    backgroundOwnerCount: result.backgroundOwnerCount,
    backgroundTerminalCount: result.backgroundTerminalCount,
    backgroundTerminalRegistryComplete: result.backgroundTerminalRegistryComplete,
    observedAtMs: result.observedAtMs,
  });
}
...
class ProcessManager {
   ...
   *   sourceMsgId: (string|number),
   * }}
   *   [opts.codexRetirementPreparer] — synchronous consumer ownership handshake;
   *   the acknowledgement echoes every input binding without normalization.
   ...
   *   — absolute deadline for the optional post-stop durability handshake.
   */
  constructor({
    processFactory,
    ...
    this._lifecycleAdmissionClosed = false;
    this._cleanRetirementCandidates = new Map();
    ...
    if (
      codexRetirementPreparer != null
      && typeof codexRetirementPreparer !== 'function'
    ) {
      throw new TypeError(
        'ProcessManager: codexRetirementPreparer function required',
      );
    }
    this.codexRetirementPreparer = codexRetirementPreparer;
    if (
      codexRetirementVerifier != null
  ...
  async inspectCleanRestartQualification(expectation, { requireFence = false } = {}) {
    const lease = this._codexLease;
    const proc = lease?.proc ?? null;
    const sessionKey = lease?.sessionKey ?? null;
    const capturedFence = {
      generationDigest: qualificationDigest(proc?.generationId),
      activityEpoch: Number.isSafeInteger(proc?.activityEpoch)
        ? proc.activityEpoch
        : null,
    };
    const parsedExpectation = normalizeQualificationExpectation(expectation, {
      requireFence,
    });
    if (!parsedExpectation.valid) {
      return qualificationResultFor(proc, 'invalid-expectation', capturedFence);
    }
    if (!proc || !this._isCodex(proc) || proc.closed || !this._codexLeaseMatches(proc)) {
      return qualificationResultFor(proc, 'no-codex-generation', capturedFence);
    }

    const { expectedGenerationDigest, expectedActivityEpoch } = parsedExpectation;
    if (
      expectedGenerationDigest != null
      && expectedGenerationDigest !== capturedFence.generationDigest
    ) {
      return qualificationResultFor(proc, 'generation-drift', capturedFence);
    }
    if (
      expectedActivityEpoch != null
      && expectedActivityEpoch !== capturedFence.activityEpoch
    ) {
      return qualificationResultFor(proc, 'activity-epoch-drift', capturedFence);
    }
    if (typeof proc.inspectCleanRestartQualification !== 'function') {
      return qualificationResultFor(proc, 'unsupported', capturedFence);
    }

    const assertLease = () => {
      if (
        this._codexLease !== lease
        || this.procs.get(sessionKey) !== proc
        || !this._codexLeaseMatches(proc)
        || proc.closed
      ) {
        throw managerError(
          'Codex qualification lease changed',
          'CODEX_QUALIFICATION_LEASE_DRIFT',
        );
      }
      if (
        qualificationDigest(proc.generationId) !== capturedFence.generationDigest
        || proc.activityEpoch !== capturedFence.activityEpoch
      ) {
        throw managerError(
          'Codex qualification fence changed',
    ...
    try {
      const result = await proc.inspectCleanRestartQualification({
        expectedGenerationDigest: capturedFence.generationDigest,
        expectedActivityEpoch: capturedFence.activityEpoch,
        assertLease,
      });
      assertLease();
      return normalizeQualificationResult(proc, result, capturedFence);
    } catch (error) {
      if (error?.code === 'CODEX_QUALIFICATION_LEASE_DRIFT') {
        return qualificationResultFor(proc, 'lease-drift', capturedFence);
      }
      if (error?.code === 'CODEX_QUALIFICATION_FENCE') {
        return qualificationResultFor(proc, 'fence-drift', capturedFence);
      }
      return qualificationResultFor(proc, 'inspection-failed', capturedFence);
    }
  }
  ...
  _withLifecycleGate(sessionKey, operation) {
    if (this._lifecycleAdmissionClosed) {
      return Promise.reject(new Error('shutdown'));
    }
    return this._enqueueLifecycleGate(sessionKey, operation);
  }

  _withInternalLifecycleGate(sessionKey, operation) {
    return this._enqueueLifecycleGate(sessionKey, operation);
  }

  _enqueueLifecycleGate(sessionKey, operation) {
  ...
  async _drainLifecycleGatesToStableEmpty(gateErrors) {
    while (true) {
      const entries = [...this._lifecycleGates.entries()];
      if (entries.length === 0) {
        await Promise.resolve();
        if (this._lifecycleGates.size === 0) return;
        continue;
      }
      await Promise.allSettled(entries.map(async ([sessionKey, gate]) => {
        try {
          await gate;
        } catch (error) {
          gateErrors.set(sessionKey, error);
        }
      }));
    }
  }
  ...
  async _spawnFresh(
    ...
    try {
      ...
      if (startupReleaseSafe) {
        ...
        if (this._lifecycleAdmissionClosed) {
          this._cleanRetirementCandidates.set(sessionKey, newProc);
        }
        this._maybeSignalLruWaiter();
      }
      throw err;
  ...
  _prepareCodexRetirement(candidate) {
    if (!this.codexRetirementPreparer) {
      throw managerError(
        'Codex continuation retirement requires durable preparation',
        'CODEX_RETIREMENT_PREPARATION_FAILED',
      );
    }
    ...
    let preparation;
    try {
      preparation = this.codexRetirementPreparer(input);
    } catch (cause) {
      throw managerError(
        'Codex continuation retirement preparation failed',
        'CODEX_RETIREMENT_PREPARATION_FAILED',
        { cause },
      );
    }
    ...
    const keys = preparation && typeof preparation === 'object'
      ? Reflect.ownKeys(preparation)
      : [];
    if (
      typeof preparation?.then === 'function'
      || keys.length !== expectedKeys.length
      || expectedKeys.some((key) => !Object.hasOwn(preparation, key))
      || preparation.committed !== true
      || preparation.disposition !== 'retirement-requested'
      || preparation.sessionKey !== input.sessionKey
      || preparation.generationId !== input.generationId
      || preparation.attemptId !== input.attemptId
      || preparation.providerSessionId !== input.providerSessionId
      || preparation.providerTurnId !== input.providerTurnId
      || preparation.sourceMsgId !== input.sourceMsgId
    ) {
      throw managerError(
        'Codex continuation retirement preparation was not exact',
        'CODEX_RETIREMENT_PREPARATION_FAILED',
      );
    }
  }
  ...
  _evictLRU() {
    ...
    this._withInternalLifecycleGate(oldestKey, async () => {
  ...
  async shutdown() {
    ...
    const retirementResults = await Promise.allSettled(targets.map((sessionKey) => (
      this._withInternalLifecycleGate(sessionKey, () => (
  ...
  async retireForCleanRestart({
    ...
    if (typeof continuationAuthorized !== 'boolean') {
      throw new TypeError(
        'ProcessManager.retireForCleanRestart: continuationAuthorized boolean required',
      );
    }
    this._shuttingDown = true;
    this._lifecycleAdmissionClosed = true;
    for (const w of this._lruWaiters) {
      clearTimeout(w.timer);
      w.reject(new Error('shutdown'));
    }
    ...
    const admittedGateErrors = new Map();
    const admittedGates = new Map(this._lifecycleGates);
    await this._drainLifecycleGatesToStableEmpty(admittedGateErrors);

    const targets = new Set([
      ...[...this.procs.entries()]
        .filter(([, proc]) => !this._retiring.has(proc))
        .map(([sessionKey]) => sessionKey),
      ...this._cleanRetirementCandidates.keys(),
      ...admittedGates.keys(),
      ...admittedGateErrors.keys(),
    ]);
    const retiredTargets = new Map();
    const qualificationLease = qualificationExpectation === undefined
      ? null
      : this._codexLease;
    const qualificationProc = qualificationLease?.proc ?? null;
    let qualification = qualificationExpectation === undefined
      ? null
      : await this.inspectCleanRestartQualification(
        qualificationExpectation,
        { requireFence: true },
      );
    ...
    let settled;
    try {
      settled = await Promise.allSettled([...targets].map(async (sessionKey) => {
        const gateError = admittedGateErrors.get(sessionKey) ?? null;
        const proc = this.procs.get(sessionKey)
          ?? this._cleanRetirementCandidates.get(sessionKey);
        if (!proc || this._retiring.has(proc)) {
          if (
            qualification?.outcome === 'qualified'
            && sessionKey === qualificationLease?.sessionKey
            && qualificationProc
          ) {
            qualification = qualificationResultFor(
              qualificationProc,
              'lease-drift',
              {
                generationDigest: qualification.generationDigest,
                activityEpoch: qualification.activityEpoch,
                observedAtMs: qualification.observedAtMs,
              },
            );
          }
        ...
        this._retiring.add(proc);
        retiredTargets.set(sessionKey, proc);
        let snapshot;
        try {
          if (this._isCodex(proc)) {
            ...
            if (qualification?.outcome === 'qualified') {
              const leaseChanged = Boolean(
                this._codexLease !== qualificationLease
                || qualificationProc !== proc
                || this.procs.get(sessionKey) !== proc
                || !this._codexLeaseMatches(proc)
                || proc.closed
              );
              const fenceChanged = Boolean(
                qualificationDigest(proc.generationId) !== qualification.generationDigest
                || proc.activityEpoch !== qualification.activityEpoch
              );
              if (leaseChanged || fenceChanged) {
                qualification = qualificationResultFor(
                  proc,
                  leaseChanged ? 'lease-drift' : 'fence-drift',
                  {
                    generationDigest: qualification.generationDigest,
                    activityEpoch: qualification.activityEpoch,
                    observedAtMs: qualification.observedAtMs,
            ...
            if (continuationAuthorized && candidate) {
              this._prepareCodexRetirement(candidate);
            }
            const retirementPromise = this._strictRetire(proc, 'clean-restart');
            retirementPromise.catch(() => {});
      ...
      for (const [sessionKey, proc] of retiredTargets) {
        if (proc.closed && this.procs.get(sessionKey) === proc) {
          this.procs.delete(sessionKey);
        }
        const candidateOwned = (
          this._cleanRetirementCandidates.get(sessionKey) === proc
        );
        if (!proc.closed && candidateOwned && !this.procs.has(sessionKey)) {
          this.procs.set(sessionKey, proc);
        }
        if (
          candidateOwned
          && (proc.closed || this.procs.get(sessionKey) === proc)
        ) {
          this._cleanRetirementCandidates.delete(sessionKey);
        }
        this._retiring.delete(proc);
      }
      this._maybeSignalLruWaiter();
    ...
    if (qualification) {
      Object.defineProperty(results, 'qualification', {
        value: qualification,
        enumerable: false,
        writable: false,
        configurable: false,
  ...
  async interrupt(sessionKey) {
    if (this._lifecycleAdmissionClosed) throw new Error('shutdown');
    const proc = this.procs.get(sessionKey);
    ...
    return this._withRetirementIntent(sessionKey, async () => {
      ...
      const interrupted = await proc.interrupt('interrupt');
  ...
  _wireCallbacks(proc) {
    ...
    let abnormalTerminationReported = false;
    ...
    const reportAbnormalTermination = (event, exitCode = null) => {
      const verifiedSafeCodexStartupRelease = (
        event === 'close'
        && this._isCodex(proc)
      ...
      if (
        abnormalTerminationReported
        || verifiedSafeCodexStartupRelease
        || this._shuttingDown
        || this._retiring.has(proc)
        || (this._retirementIntents.get(proc.sessionKey) ?? 0) > 0
        || !isCurrentGeneration()
      ) return;
      abnormalTerminationReported = true;
      const callback = this.callbacks.onAbnormalTermination;
      ...
      try { callback(proc.sessionKey, evidence, proc); }
      catch (error) {
        this.logger.error?.(
          `[pm:${proc.label}] onAbnormalTermination threw: ${error.message}`,
        );
      }
    };
    ...
    proc.on('close', (...args) => {
      reportAbnormalTermination('close', args[0]);
      if (!this._isCodex(proc)) {
        if (
          !this._retiring.has(proc)
          && this.procs.get(proc.sessionKey) === proc
      ...
      const lease = this._codexLease;
      this._withInternalLifecycleGate(proc.sessionKey, async () => {
    ...
    proc.on('bridge-disconnected', () => {
      reportAbnormalTermination('bridge-disconnected');
