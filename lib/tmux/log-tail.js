...
class LogTail extends EventEmitter {
  ...
  async _readNew(maxOffset = null) {
    ...
    const observedSize = maxOffset == null ? stat.size : Math.min(stat.size, maxOffset);
    if (observedSize < this._offset) {
    ...
    if (observedSize <= this._offset) return; // unchanged
    const fd = await this.fs.promises.open(this.path, 'r');
    try {
      const bytesToRead = observedSize - this._offset;
  ...
  async drain() {
    if (this._closed) throw new Error('LogTail: cannot drain a closed tail');
    this._draining = true;
    while (this._readInFlight) {
      await new Promise(resolve => setImmediate(resolve));
    }
    let watermark;
    try {
      watermark = (await this.fs.promises.stat(this.path)).size;
    } catch (err) {
      if (err.code !== 'ENOENT') {
        this._draining = false;
        throw err;
    ...
    this._readInFlight = true;
    try {
      await this._readNew(watermark);
    } finally {
      this._readInFlight = false;
      this._draining = false;
    }
    return Object.freeze({ offset: this._offset, watermark });
  }
