...
async function readSnapshot(filePath, byteSize, {
  ...
  if (byteSize > MAX_WORKFLOW_TRANSCRIPT_BYTES) {
    return { ok: false, reason: 'snapshot-too-large' };
  }
  const validatesPrefix = prefixByteSize !== null || prefixHash !== null;
  if (
    validatesPrefix
    && (
      !Number.isSafeInteger(prefixByteSize)
      || prefixByteSize <= 0
    ...
    return { ok: false, reason: 'snapshot-prefix-invalid' };
  }
  const validatesIdentity = fileDevice !== null || fileInode !== null;
  if (
    validatesIdentity
    && (
      typeof fileDevice !== 'string'
      || !/^\d+$/.test(fileDevice)
      || typeof fileInode !== 'string'
      || !/^\d+$/.test(fileInode)
    )
  ) {
    return { ok: false, reason: 'snapshot-identity-invalid' };
  }
  let handle;
  try {
    ...
    if (
      ...
      || (
        validatesIdentity
        && (
          before.dev.toString() !== fileDevice
          || before.ino.toString() !== fileInode
        )
      )
    ) {
      return { ok: false, reason: 'snapshot-replaced' };
    }
    if (before.size < BigInt(byteSize)) {
      return { ok: false, reason: 'snapshot-truncated' };
    }
    ...
    if (
      ...
      return { ok: false, reason: 'snapshot-changed-during-read' };
    }
    if (
    ...
    if (
      validatesPrefix
      && crypto.createHash('sha256').update(buffer.subarray(0, prefixByteSize)).digest('hex') !== prefixHash
    ) {
      return { ok: false, reason: 'snapshot-prefix-changed' };
    }
...
async function correlateWorkflowCompletionSnapshot({
  ...
  const ancestryRows = rows.filter(row => (
    row.sessionId === sessionId && row.isSidechain === false
  ));
  ...
  if (terminalIndex !== mainline.length - 1) {
    return result('terminal-not-last-mainline', {
