    ...
    if (existing && !existing.closed) {
      ...
      if (this._isCodex(existing) && existing.state === 'Stopped') {
        ...
        this._logEvent('cli-config-reload', {
          ...
          // Which dimension(s) drifted, comma-joined — soak reads this to tell a
          // per-chat rendering toggle from a /model or /effort change.
          reason: typeof existing.reloadReasonsFor === 'function'
            ? existing.reloadReasonsFor(spawnContext).join(',')
            : null,
        });
