...
function staticThreadPolicy(source) {
  return {
...
class CodexProcess extends Process {
  ...
  _validateThreadResult(result, expectedThreadId) {
    ...
    this._validateAndObserveThreadPolicy(result, { attaching: true });
  }

  _validateAndObserveThreadPolicy(source, { attaching = false } = {}) {
    const actualStaticPolicy = staticThreadPolicy(source);
    if (!isDeepStrictEqual(actualStaticPolicy, this.expectedStaticPolicy)) {
      throw processError(
  ...
  async _transportCheckpoint(kind, detail, transport, onCommitted = null) {
    ...
    try {
      ...
      if (kind === 'request-write-attempted') markWriteCommitted();
      transport.assertActive?.();
    } catch (error) {
      const durability = processError(
