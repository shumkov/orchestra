...
class CliProcess extends Process {
  ...
  _settleDeliveryAttempt(entry, outcome) {
    if (
      ...
      entry.settle({ outcome: 'invalidated' });
      return;
    }
  ...
  _invalidatePendingDeliveryWork() {
    ...
    for (const entry of this._deliveryAttempts.values()) {
      if (entry.outcome === 'pending') entry.settle({ outcome: 'invalidated' });
    }
  ...
  async _dispatchToolCall(msg) {
    ...
    if (msg.name === 'reply' && typeof args.text === 'string' && args.chat_id != null) {
      const dedupKey = this._buildContentDedupKey(args);
    ...
    if (msg.name === 'reply' && typeof args.text === 'string' && args.chat_id != null) {
      inflightKey = this._buildContentDedupKey(args);
  ...
  _handleHookEvent(ev) {
    ...
    switch (ev.type) {
      ...
      case 'Stop': {
        ...
        const workflowSnapshot = this._captureWorkflowCompletionSnapshot(ev);
        const info = {
