...
// Bound on the per-turn snapshot counters kept in memory. Foreign turn_ids get
// a counter too (they're counted before attribution), so the map is FIFO-capped
// rather than tied to turn lifecycle.
const STREAM_TURN_COUNTER_LIMIT = 64;
...
class CliProcess extends Process {
  ...
  constructor({
    ...
    // turn_id → live-preview snapshots accepted for it (anti-runaway bound).
    this._streamCallCounts = new Map();
  ...
  _bridgeEnv() {
    return {
      ...
      ...(this.streamToolEnabled ? { ORCHESTRA_STREAM_TOOL: '1' } : {}),
    };
  }
  ...
  _handleStreamCall(msg, args) {
    ...
    const calls = (this._streamCallCounts.get(turnId) || 0) + 1;
    this._streamCallCounts.set(turnId, calls);
    while (this._streamCallCounts.size > STREAM_TURN_COUNTER_LIMIT) {
      this._streamCallCounts.delete(this._streamCallCounts.keys().next().value);
    }
    if (calls > STREAM_CALLS_PER_TURN_CAP) {
      // One event per turn, not per dropped call — a runaway would otherwise
      // write thousands of rows describing the same runaway.
      if (calls === STREAM_CALLS_PER_TURN_CAP + 1) {
