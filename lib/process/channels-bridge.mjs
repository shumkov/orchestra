...
mcp.setRequestHandler(ListToolsRequestSchema, async () => {
  ...
  tools: [{
    ...
    name: 'edit_message',
    description: 'Edit a message you previously sent via `reply`, in place. The edit is ' +
                 're-rendered exactly as the reply was, so a checklist/table/heading comes ' +
                 'back rich rather than flat. Canonical use: send a plan as `- [ ] step` ' +
                 'items, then edit that same message_id with the WHOLE list again and `- [x]` ' +
                 'on completed steps — check items off as you go and the user watches one ' +
                 'bubble update. Also fine for a short one-line status. An edit REPLACES the ' +
                 'body, so always re-send the full text. Keep status in PLAIN LANGUAGE — never ' +
                 'tool names like Bash/Edit. One bubble, never chunked: ~4,000 chars for plain ' +
                 'text, much more (~32,000) when it renders rich; over the limit the tool tells ' +
                 'you which one applied — send long content as a fresh `reply` instead. ' +
                 'An edit does NOT notify the user and carries no consumed_turn_ids: it is a ' +
                 'mid-turn update, never the final answer.',
    inputSchema: {
      type: 'object',
      properties: {
        chat_id:    { type: 'string', description: 'Echo of chat_id from inbound channel meta.' },
