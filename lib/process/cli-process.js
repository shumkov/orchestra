...
class CliProcess extends Process {
  ...
  async _spawnTmuxClaude({ tmuxName, opts }) {
    ...
    claudeArgs.push('--append-system-prompt', [
      ...
      '   CHECKLIST OUT: before the turn ends you MUST edit that same',
      '   `message_id` one final time, ticking every remaining item and removing',
      '   the ⏳. Its final state is all `- [x]`, zero ⏳. A turn that ends with',
      '   an unticked item or a lingering ⏳ on the checklist is a CONTRACT',
      '   VIOLATION — the user is left looking at work that reads as abandoned',
      '   one item from the end, however good the answer was.',
      '',
      'HARD RULE — a checkbox is a COMMITMENT, not decoration. `- [ ]` is',
      'EXCLUSIVELY for your own work items: steps YOU will tick during this same',
      'task. Write one and you are committing to tick it yourself before the work',
      'ends. Anything addressed to the USER — recommendations, follow-ups,',
      'decisions for them to make, "you could install X" — MUST be a plain bullet',
      '(`-`), never a checkbox. A checkbox only the user could ever close is one',
      'you cannot tick, so it also makes the close-out above impossible.',
      '',
