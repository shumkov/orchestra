...
function createCodexProcess({
  ...
  const strictResume = ctx?.resumePolicy === 'require-interrupted-turn';
  if (
    ctx?.resumePolicy != null
    && !strictResume
  ) {
    throw codexConfigurationError('unsupported Codex resume policy');
  }
  if (!strictResume && ctx?.expectedInterruptedTurnId != null) {
    throw codexConfigurationError(
      'interrupted turn requires strict Codex resume policy',
    );
  }
  if (
    strictResume
    && (
      ...
      || !isBoundedOpaqueId(ctx?.expectedInterruptedTurnId)
    )
  ) {
    throw codexConfigurationError(
      'strict Codex resume requires an existing thread and interrupted turn',
    );
  }
  const spawnOptions = Object.freeze({
    ...
    ...(strictResume ? {
      ...
      expectedInterruptedTurnId: ctx.expectedInterruptedTurnId,
    } : {}),
  ...
  const proc = new CodexProcess({
    ...
    // Only a profile that carries hook material asks for verification; without
    // it the session runs the request sequence it always ran.
    ...(profile.hookManifest == null ? {} : {
      hookVerifier: (client, phase) => client.verifyHooks({ phase }),
    }),
    checkpointSink,
    hostIdentity,
    bootSessionIdentity,
 ...
 * @param {object|Function} [opts.toolDispatcherCapabilities] — optional bridge tools this
 *   consumer's dispatcher can honor, e.g. `{stream: true}` for a live-preview surface.
 *   Accepts an object (same for every session) OR a resolver
 *   `(chatId, threadId, config) => object` invoked once per spawn, so the capability can
 *   be rolled out per chat/topic. Unset → the optional tools are never registered and
 *   consumers that don't implement them are unaffected.
 ...
 *   engine knowing anything about the consumer's config shape. Only the 'cli'
 *   backend uses this; the 'sdk' backend gets its per-chat hint via spawnFn instead.
 *
 *   A `spawnContext.displayHint` string OVERRIDES both forms, and a consumer whose
 *   hint can change must send one: the hint is spawn-time state, so a change reaches
 *   a warm session only through getOrSpawn's reload-on-drift, which compares the
 *   context's hint against the warm proc's. Passing the identical resolved string on
 *   the context makes the reload and the respawn one source — resolving independently
 *   here would make them two, and a one-character disagreement would respawn the
 *   session on every message forever. The resolver remains the fallback for consumers
 *   that send no hint on the context.
  ...
  return function processFactory(sessionKey, ctx) {
    ...
    if (choice === 'cli') {
      ...
      if (missing.length) {
        ...
        // The spawn context's hint wins whenever it carries one. getOrSpawn's
        // reload-on-drift compares THAT string against the warm proc's, so the
        // respawn must carry the same one — resolving independently here would
        // make apply-path and detect-path two sources, and a one-character
        // disagreement between them respawns the session on every message,
        // forever. The resolver stays the fallback for consumers that send no
        // hint on the context: a static string (same hint for every session) or
        // a function called per spawn with the current chat/topic and config, so
        // the consumer can vary the hint per chat (e.g. a rich-text toggle).
        const resolvedDisplayHint = typeof ctx?.displayHint === 'string'
          ? ctx.displayHint
          : typeof displayHint === 'function'
            ? displayHint(chatId, threadId, config)
            : displayHint;
        // Same string-or-resolver shape as displayHint: the optional tools a
        // consumer can honor may vary per chat/topic (a live preview is a
        // per-chat rollout decision), and the prompt section that teaches them
        // is resolved at the same moment, so the two can never disagree.
        const resolvedCapabilities = typeof toolDispatcherCapabilities === 'function'
          ? toolDispatcherCapabilities(chatId, threadId, config)
          : toolDispatcherCapabilities;
        return new CliProcess({
          ...
          sessionLauncher,
          toolDispatcher,
          toolDispatcherCapabilities: resolvedCapabilities,
