...
class CliProcess extends Process {
  ...
  async _spawnTmuxClaude({ tmuxName, opts }) {
    ...
    claudeArgs.push('--append-system-prompt', [
      ...
      '`message_id` to update the bubble as you progress.',
      '',
      ...
      'flat. So on multi-step work the strongest progress bubble is a CHECKLIST:',
      'send the plan as `- [ ] step` items, then `edit_message` that same message_id',
      'with the list again and `- [x]` on what you have finished. Check items off as',
      'you complete them — the user watches ONE bubble fill in instead of collecting',
      'a trail of status messages. Always re-send the WHOLE list (an edit REPLACES',
      'the body, so a partial re-send loses everything you left out).',
      '',
      'Short one-line status text is still fine for an interim bubble — a checklist',
      'is the better shape when the work has steps worth showing, not a requirement.',
      '',
