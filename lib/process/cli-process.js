...
class CliProcess extends Process {
  ...
  _captureWorkflowCompletionSnapshot(ev) {
    ...
    try {
      const stat = fs.statSync(ev.transcriptPath);
  ...
  _beginWorkflowCompletionDecision(snapshot) {
    ...
    const decisionKey = crypto.createHash('sha256')
      .update(`${snapshot.transcriptPath}\0${snapshot.byteSize}\0${normalizeVisibleText(snapshot.finalText)}`, 'utf8')
      .digest('hex');
    const now = Date.now();
    ...
    Promise.resolve()
      .then(() => correlateWorkflowCompletionSnapshot(snapshot))
