...
const ToolCallMessageSchema = z.object({
  ...
  //
  // 'stream': a mid-turn snapshot of the answer the agent is composing, for the
  // consumer's live preview. Args are {chat_id, turn_id, text}; turn_id is
  // REQUIRED (a snapshot that can't be attributed to a turn is dropped, never
  // painted onto whatever turn happens to be in flight). It is NOT a delivery —
  // the daemon forwards it as a `stream-chunk` event and never routes it
  // through the reply/delivery paths. Registered on the bridge only when the
  // consumer opted in (ORCHESTRA_STREAM_TOOL).
  name: z.enum(['reply', 'react', 'edit_message', 'ask', 'stream']),
...
const ToolAckMessageSchema = z.object({
  ...
  attempt_id: NonEmptyString.optional(),
  delivery: z.enum(['sent', 'replayed']).optional(),
  replay_of: NonEmptyString.optional(),
...
function formatToolAckResult({ ack, error, toolCallId }) {
  if (error) {
    return {
      content: [{
        type: 'text',
        text: JSON.stringify({
          ok: false,
          error: error.message,
          timeout: error.code === 'TOOL_ACK_TIMEOUT',
          attempt_id: toolCallId,
        }),
      }],
      isError: true,
    };
  ...
  if (ack?.ok !== true) {
    return {
      content: [{
        ...
        text: JSON.stringify({
          ok: false,
          error: ack?.error || 'daemon rejected delivery',
          attempt_id: ack?.attempt_id || toolCallId,
        }),
      }],
      isError: true,
    };
  ...
  return {
    content: [{
      ...
      text: JSON.stringify({
        ok: true,
        message_id: ack.message_id ?? null,
        attempt_id: ack.attempt_id,
        delivery: ack.delivery,
        ...(ack.replay_of ? { replay_of: ack.replay_of } : {}),
      }),
    }],
  };
}
