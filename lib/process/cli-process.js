...
} = require('./workflow-completion-correlation');

const BRIDGE_PATH = path.resolve(__dirname, 'channels-bridge.mjs');
const DEFAULT_HANDSHAKE_TIMEOUT_MS = 15_000;
// Claude-side MCP registration can lag well behind daemon-side bridge
// authentication on cold starts. Keep this below the wider TUI startup bound
// while leaving enough room for a healthy bridge to finish ListTools.
...
const WORKFLOW_TRANSCRIPT_SETTLE_MS = 1_000;
const WORKFLOW_TRANSCRIPT_POLL_MS = 25;
...
// Live-preview snapshots a single turn may push. Deliberately NOT part of the
// reply token bucket: the consumer's own edit throttle is the real rate limiter,
// and sharing a bucket would let a burst of snapshots starve the `reply` that
// ends the turn. This is only an anti-runaway bound.
const STREAM_CALLS_PER_TURN_CAP = 200;

function validateSessionLauncher(sessionLauncher) {
  if (!sessionLauncher) return null;
  if (typeof sessionLauncher !== 'string' || !path.isAbsolute(sessionLauncher)) {
    throw Object.assign(new TypeError('CliProcess: sessionLauncher must be an absolute executable path'), {
      code: 'SESSION_LAUNCHER_INVALID',
    });
  }
  try {
    if (!fs.statSync(sessionLauncher).isFile()) {
      throw new Error('not a regular file');
    }
    fs.accessSync(sessionLauncher, fs.constants.X_OK);
  } catch {
    throw Object.assign(new TypeError(`CliProcess: sessionLauncher is not an executable file: ${sessionLauncher}`), {
      code: 'SESSION_LAUNCHER_INVALID',
    });
  }
...
class CliProcess extends Process {
  ...
  constructor({
    ...
    toolDispatcher,
    // Which optional bridge tools this consumer's dispatcher can honor.
    // `{stream: true}` registers the `stream` tool + its prompt section and
    // turns snapshots into `stream-chunk` events. A consumer with no live
    // preview surface leaves it unset and the tool never exists — the boundary
    // that can actually skew is this wiring, not bridge↔daemon (they ship in
    // one package), which is why the gate lives here and not in a handshake.
    toolDispatcherCapabilities = null,
    ...
    this.sessionLauncher = validateSessionLauncher(sessionLauncher);
    this.toolDispatcher = toolDispatcher;
    this.streamToolEnabled = toolDispatcherCapabilities?.stream === true;
    // What claude calls the live-preview tool in its hook stream. The
    // PreToolUse/PostToolUse matcher is `.*`, so the agent's own stream calls
    // arrive here as work hooks — see _handleHookEvent for why that matters.
    this._streamToolHookName = `mcp__${bridgeServerName}__stream`;
    // Normalized to a string: the drift check compares with !== against the
    // spawn context's string hint, so a null (from a resolver that returned
    // one) would be drift no context could ever match — a permanent respawn.
    this.displayHint = typeof displayHint === 'string' ? displayHint : '';
    ...
    this._bridgeDisconnectErrorPromise = null;
    ...
    this.mcpReady = false;
    this._cancelBridgeHandshakeWait = null;
    this._bridgeHandshakeCancellationError = null;
    this.startupPingTimer = null;
    this.pingTimer = null;
    ...
    this.recentToolCallResults = new Map();   // tool_call_id → successful direct-dispatch receipt
    ...
    this.recentContentHashes = new Map();   // key → { expiry, message_id, attempt_id }
    ...
    // Only unresolved dispatcher attempts and transcript decisions are
    // lifecycle state. Settled delivery authority lives in Claude's transcript;
    // the bounded outcome ledger exists solely to resolve a bridge timeout that
    // names the same opaque attempt id and immutable argument hash.
    this._deliveryAttempts = new Map();
    this._deliveryAttemptOrder = [];
    this._workflowDecisionKeys = new Map();
    this._activeWorkflowDecisions = new Set();
    ...
    this._openApprovals = new Set();
    this._questionKeepAliveTimer = null;
    this._cleanRestartOutputFenced = false;
    this._cleanRestartEvidenceLost = false;
    this._resumeAttestation = null;
    Object.defineProperty(this, 'resumeAttestation', {
      enumerable: true,
      configurable: false,
      get: () => this._resumeAttestation,
    });
  ...
  async start(opts = {}) {
    ...
    this._resumeAttestation = null;
    this._pendingResumeAttestation = null;
    this._strictExpectedSessionId = opts.resumePolicy === 'require-existing-session'
    ...
    try {
      ...
      if (this._strictExpectedSessionId != null) {
        if (this.claudeSessionId !== this._strictExpectedSessionId) {
          throw Object.assign(
            new Error('strict resume reported a different session identity'),
            { code: 'REQUIRED_SESSION_MISMATCH' },
          );
        }
        this._resumeAttestation = this._pendingResumeAttestation;
      }
      this._stopStartupPingLoop();
  ...
  async _createSocketServer() {
    ...
    this.bridgeServer.on('session-init', msg => {
      if (!this._acceptBridgeSessionId(msg.claude_session_id)) return;
    ...
    this.bridgeServer.on('bridge-disconnected', () => {
      this._handleBridgeDisconnected().catch(err => {
        this.logger.error?.(`[${this.label}] channels: bridge disconnect handling failed: ${err.message}`);
      });
    });
  ...
  _acceptBridgeSessionId(sessionId) {
    if (
      this._strictExpectedSessionId != null
      && sessionId !== this._strictExpectedSessionId
    ) {
      this._resumeAttestation = null;
      this._pendingResumeAttestation = null;
      this._cancelPendingBridgeHandshake(
        'REQUIRED_SESSION_MISMATCH',
        'strict resume reported a different session identity',
      );
      return false;
    }
    return true;
  ...
  _bridgeEnv() {
    return {
      ...
      // Opt-in live-preview tool. Set at spawn time so the registered tool set
      // and the system prompt can never disagree about whether `stream` exists.
      //
      // ALWAYS written, never merely omitted: the bridge reads the ambient
      // environment, and this env travels daemon → tmux → claude → MCP child.
      // An ORCHESTRA_STREAM_TOOL=1 anywhere up that chain (an operator's shell,
      // a co-tenant daemon) would otherwise switch the tool on for a consumer
      // that never opted in. Omission is not a decision; '' is.
      ORCHESTRA_STREAM_TOOL: this.streamToolEnabled ? '1' : '',
    };
  }
  ...
  async _spawnTmuxClaude({ tmuxName, opts }) {
    ...
    let canResume = false;
    let resumePath = null;
    const strictResume = opts.resumePolicy === 'require-existing-session';
    if (
      strictResume
      && (
        typeof opts.expectedSessionId !== 'string'
        || !opts.expectedSessionId
        || opts.expectedSessionId !== opts.existingSessionId
      )
    ) {
      throw Object.assign(
        new Error('strict resume requires expectedSessionId to match existingSessionId'),
        { code: 'REQUIRED_SESSION_MISMATCH' },
      );
    }
    if (opts.existingSessionId && resolvedCwd) {
      // Claude replaces path separators and dots in project directory names.
    ...
    if (strictResume && !canResume) {
      throw Object.assign(
        new Error(
          `required session ${opts.expectedSessionId} is unavailable at ${resumePath || '<unknown cwd>'}`,
        ),
        { code: 'REQUIRED_SESSION_NOT_FOUND' },
      );
    }
    if (canResume) {
      claudeArgs.push('--resume', opts.existingSessionId);
      this._pendingResumeAttestation = Object.freeze({
    ...
    claudeArgs.push('--append-system-prompt', [
      ...
      'include the id.',
      '',
      '### Staying responsive on a long task — HARD CONTRACT',
      '',
      'The user sees NOTHING while you work — no inline text, no tool output reaches',
      'them. A turn that runs long with no visible progress looks BROKEN, and',
      'working in silence then dropping a finished result is a CONTRACT VIOLATION,',
      'not a style choice: the user sat staring at a dead chat the whole time.',
      '',
      'So for any MULTI-STEP task — several steps, research, more than a couple of',
      'tool calls — this is the sequence, every time:',
      '',
      '1. Your FIRST visible action MUST be a `reply` with `interim: true` whose',
      '   body is the PLAN as a checklist: one `- [ ] step` line per step, in plain',
      '   task language. It returns a `message_id` — keep it.',
      `2. Posting the plan is NOT a stopping point. Do not end the turn to wait`,
      '   for approval — the checklist is a progress display, not a proposal. Keep',
      '   executing in the SAME turn immediately (ask first only when a step is',
      '   destructive or the request is genuinely ambiguous).',
      `3. After EVERY step you complete — not at the end, after each one — you MUST`,
      `   call \`mcp__${b}__edit_message\` on that SAME \`message_id\` with the WHOLE`,
      '   list again — check items off as you complete them: finished items flip',
      '   to `- [x]`. Five steps means the bubble changes five times; the user',
      '   watches ONE message fill in, live.',
      '4. Always re-send the WHOLE list — an edit REPLACES the body, so a partial',
      '   re-send loses everything you left out.',
      '5. Mark the item you are working on RIGHT NOW by making its text BOLD:',
      '   `- [ ] **Проверяю конфиги…**` — the whole item text wrapped in `**`.',
      '   Exactly one item is bold at a time; when the item completes it becomes',
      '   plain `- [x]` and the bold moves to the next one. The user sees at a',
      '   glance what is happening right now.',
      '6. Delivering the final answer COMPLETES the last step, so CLOSE THE',
      '   CHECKLIST OUT: immediately after that final `reply`, edit that same',
      '   `message_id` one last time, ticking every remaining item and removing',
      '   the bold. Its final state is all `- [x]`, nothing bold. A turn that',
      '   ends with a lingering bold item on the checklist is a CONTRACT',
      '   VIOLATION — the user is',
      '   left looking at work that reads as abandoned one item from the end,',
      '   however good the answer was.',
      '   But NEVER tick an item you did not actually complete — a false `- [x]`',
      '   is worse than an unfinished list. When work stops early — a step fails,',
      '   is skipped, turns out to be blocked — the close-out edit still happens:',
      '   remove the bold, leave that item unticked, and name the reason ON its',
      '   line, e.g. `- [ ] Деплой — заблокировано: нет доступа к VPS`. The',
      '   lingering bold is the violation; an honestly labelled unfinished item',
      '   is not.',
      '',
      'HARD RULE — a checkbox is a COMMITMENT, not decoration, and this applies to',
      'EVERY message you send, the final answer included, not just the progress',
      'checklist. `- [ ]` is EXCLUSIVELY for your own work items: steps YOU will',
      'tick during this same task. Write one and you are committing to tick it',
      'yourself before the work ends. Anything addressed to the USER —',
      'recommendations, follow-ups, decisions for them to make, "you could install',
      'X" — MUST be a plain bullet (`-`), never a checkbox. A checkbox only the',
      'user could ever close is one you cannot tick, so it also makes the close-out',
      'above impossible. Putting a checkbox on something only the user can act on',
      'is a CONTRACT VIOLATION.',
      '',
      'An edit re-renders the bubble exactly as the original `reply` did — a chat',
      'that renders rich re-renders an edited checklist, table or heading rich, not',
      'flat. A short one-line status bubble is acceptable only when the work',
      'genuinely has no enumerable steps.',
      '',
      'An edit is TEXT-ONLY. Checklists, tables and headings re-render; image',
      'markdown degrades to its caption, and an edit can never ADD media to a',
      `bubble — send files with \`reply(files:)\`. An edit that is nothing but`,
      'media has nothing left to say and will come back as an error.',
      '',
      'Length: an edit is ONE bubble, never chunked. Plain text is capped around',
      '4,000 characters; a rich-rendered structure in a rich-enabled chat gets a',
      'much larger one (~32,000). You cannot see which applies — if you go over, the',
      'tool tells you the limit that applied, so shorten the edit or send the long',
      'content as a fresh `reply`.',
      '',
      'HARD RULE — a status bubble or progress checklist is a MID-TURN update, NOT',
      'the end of work, and `edit_message` NEVER delivers the final answer. After an',
      ...
      '',
      // Present ONLY when the consumer registered the tool (same spawn-time flag
      // that puts `stream` in the bridge's tool list), so the contract below can
      // never coach the agent toward a tool that isn't there.
      ...(this.streamToolEnabled ? [
        '### Showing your answer as you write it — HARD CONTRACT',
        '',
        'The user is watching a LIVE bubble the whole time you compose, and',
        `\`mcp__${b}__stream\` is the ONLY thing that fills it in. Writing a long`,
        'answer in silence and dropping it at the end is a CONTRACT VIOLATION, not a',
        'style choice: the user sat watching an empty bubble for the entire time you',
        'were writing.',
        '',
        'So if your answer will run longer than about two paragraphs, this is the',
        'cadence: after EVERY section you finish — or after every paragraph, when',
        'the answer has no sections — and BEFORE you start writing the next one,',
        `you MUST call \`mcp__${b}__stream\` with the FULL answer-so-far.`,
        'Not once at the end, not "at a natural checkpoint" — after each one,',
        'every time. Four sections means four stream calls, and then the reply.',
        '',
        'Each call REPLACES the preview, so always send the whole answer up to that',
        'point — never just the part you added.',
        '',
        'HARD RULE — `stream` is a PREVIEW, not a delivery. It never ends a turn and',
        `it is not the answer. ALWAYS finish with a normal \`mcp__${b}__reply\``,
        'carrying the complete answer; NEVER end a turn on a stream call.',
        '',
        'A couple of sentences is short — just reply, no stream. And never retry a',
        'stream call: it cannot fail in a way that matters.',
        '',
        'Media never enters a streamed message. A streamed bubble finalizes by',
        'editing, and an edited bubble can never gain media — photos degrade to',
        'their caption text. When your answer includes media (inline images, a',
        'collage, a slideshow), SKIP streaming for that answer entirely: compose',
        'it and send ONE normal reply, where the media renders inline in the',
        'rich document, pictures in place. Do NOT stream the prose first and do',
        'NOT split the media into a separate message.',
        '',
      ] : []),
    ...
    await this.runner.spawn({
      ...
      command: this.sessionLauncher || this.claudeBin,
      args: this.sessionLauncher ? [this.claudeBin, ...claudeArgs] : claudeArgs,
      envExtras: {
  ...
  _waitForBridgeHandshake() {
    return new Promise((resolve, reject) => {
      if (this._bridgeHandshakeCancellationError) {
        reject(this._bridgeHandshakeCancellationError);
        return;
      }
      if (this._bridgeDisconnecting || this._bridgeDisconnectErrorPromise) {
        const err = new Error('bridge disconnected before MCP registration completed');
      ...
      let mcpTimer = null;
      const maybeResolve = () => {
        if (this.bridgeReady && this.mcpReady) settle(resolve);
      };
      const onBridgeReady = () => {
        this.bridgeReady = true;
        maybeResolve();
      ...
      const cleanup = () => {
        ...
        this.off('bridge-ready', onBridgeReady);
        this.off('mcp-ready', onMcpReady);
        bridgeServer?.off?.('bridge-disconnected', onBridgeDisconnected);
        if (this._cancelBridgeHandshakeWait === cancelWait) {
          this._cancelBridgeHandshakeWait = null;
        }
      };
      ...
      const cancelWait = (error) => settle(reject, error);
      this._cancelBridgeHandshakeWait = cancelWait;
      if (!this.bridgeReady) {
        this.on('bridge-ready', onBridgeReady);
      ...
      bridgeServer?.once?.('bridge-disconnected', onBridgeDisconnected);
      maybeResolve();
    });
  }

  _cancelPendingBridgeHandshake(code, message) {
    const error = new Error(message);
    error.code = code;
    this._bridgeHandshakeCancellationError = error;
    this._cancelBridgeHandshakeWait?.(error);
  }
  ...
  _handleBridgeMessage(msg) {
    switch (msg.kind) {
      ...
      case 'perm_req':
        this._openApprovals.add(msg.request_id);
  ...
  /**
   * Stable dedup key for immutable reply delivery arguments. Including the
   * destination prevents identical content sent to different chats from
   * colliding.
   */
  _buildContentDedupKey(args) {
    return hashDeliveryArguments('reply', args);
  }

  _pruneDeliveryAttempts(now = Date.now()) {
    for (const [attemptId, entry] of this._deliveryAttempts) {
      if (entry.outcome !== 'pending' && entry.expiry <= now) {
        this._deliveryAttempts.delete(attemptId);
      }
    }
    while (this._deliveryAttemptOrder.length > RECENT_TOOL_CALL_LIMIT) {
      const attemptId = this._deliveryAttemptOrder.shift();
      const entry = this._deliveryAttempts.get(attemptId);
      if (entry?.outcome !== 'pending') this._deliveryAttempts.delete(attemptId);
    }
  }

  _registerDeliveryAttempt(attemptId, argumentHash) {
    ...
    this._deliveryAttempts.set(attemptId, entry);
    return entry;
  }

  _settleDeliveryAttempt(entry, outcome) {
    if (
      entry.generation !== this._deliveryGeneration
      || this._deliveryAttempts.get(entry.attemptId) !== entry
    ) {
      entry.settle();
      return;
    }
    entry.outcome = outcome;
    entry.expiry = Date.now() + this.contentDedupWindowMs;
    entry.settle();
    this._deliveryAttemptOrder.push(entry.attemptId);
    this._pruneDeliveryAttempts();
    this.emit('delivery-work-settled');
  }

  async _readDeliveryAttempt(attemptId, argumentHash, generation) {
    const entry = this._deliveryAttempts.get(attemptId);
    if (!entry || entry.generation !== generation) return { outcome: 'missing' };
    if (entry.argumentHash !== argumentHash) return { outcome: 'mismatch' };
    if (entry.outcome === 'pending') await entry.promise;
    if (generation !== this._deliveryGeneration) return { outcome: 'invalidated' };
    const current = this._deliveryAttempts.get(attemptId);
    if (!current || current.argumentHash !== argumentHash) return { outcome: 'missing' };
    return { outcome: current.outcome };
  }

  _invalidatePendingDeliveryWork() {
    const hadPending = this.hasPendingDeliveryWork();
    this._deliveryGeneration++;
    for (const entry of this._deliveryAttempts.values()) {
      if (entry.outcome === 'pending') entry.settle();
    }
    this._deliveryAttempts.clear();
    this._deliveryAttemptOrder.length = 0;
    this._activeWorkflowDecisions.clear();
    this._workflowDecisionKeys.clear();
    if (hadPending) this.emit('delivery-work-settled');
  }

  _captureWorkflowCompletionSnapshot(ev) {
    if (
      typeof ev.transcriptPath !== 'string'
      || !ev.transcriptPath
      || typeof ev.lastAssistantMessage !== 'string'
      || !normalizeVisibleText(ev.lastAssistantMessage)
    ) return null;
    let fd;
    try {
      fd = fs.openSync(ev.transcriptPath, 'r');
      const before = fs.fstatSync(fd, { bigint: true });
      if (
        !before.isFile()
        || before.size <= 0n
        || before.size > BigInt(MAX_WORKFLOW_TRANSCRIPT_BYTES)
      ) return null;
      const byteSize = Number(before.size);
      const buffer = Buffer.allocUnsafe(byteSize);
      let offset = 0;
      while (offset < byteSize) {
        const bytesRead = fs.readSync(fd, buffer, offset, byteSize - offset, offset);
      ...
      return {
        ...
        prefixHash: crypto.createHash('sha256').update(buffer).digest('hex'),
        fileDevice: before.dev.toString(),
        fileInode: before.ino.toString(),
        sessionId: ev.sessionId || this.claudeSessionId,
        finalText: ev.lastAssistantMessage,
        workHookSeq: this._workHookSeq || 0,
      };
    } catch {
      return null;
    } finally {
  ...
  async _correlateSettledWorkflowSnapshot(snapshot, generation) {
    ...
    const invalidated = () => (
      generation !== this._deliveryGeneration || this.closed
    );
    const identityChanged = stat => (
      stat.dev.toString() !== snapshot.fileDevice
      || stat.ino.toString() !== snapshot.fileInode
    );
    const verifyEligible = async correlation => {
      if (
        !Number.isSafeInteger(correlation.snapshotByteSize)
        || typeof correlation.snapshotHash !== 'string'
      ) {
        return ineligible('snapshot-proof-missing');
      }
      const verified = await correlateWorkflowCompletionSnapshot({
        ...snapshot,
        byteSize: correlation.snapshotByteSize,
        prefixByteSize: correlation.snapshotByteSize,
        prefixHash: correlation.snapshotHash,
      });
      if (invalidated()) return null;
      if (
        !verified.eligible
        || verified.snapshotHash !== correlation.snapshotHash
      ) {
        return ineligible('snapshot-changed-before-decision');
      }
      return verified;
    };

    let correlation = await correlateWorkflowCompletionSnapshot(snapshot);
    if (invalidated()) return null;
    if (correlation.eligible) return verifyEligible(correlation);
    if (correlation.reason !== 'terminal-not-last-mainline') return correlation;
    if (typeof correlation.snapshotHash !== 'string') {
      return ineligible('snapshot-prefix-missing');
    }

    const prefixHash = snapshot.prefixHash;
    const deadline = Date.now() + WORKFLOW_TRANSCRIPT_SETTLE_MS;
    let observedSize = snapshot.byteSize;
    while (Date.now() < deadline) {
      if (invalidated()) return null;
      if ((this._workHookSeq || 0) !== snapshot.workHookSeq) {
        return ineligible('intervening-work');
      }

      let stat;
      try {
        stat = await fs.promises.stat(snapshot.transcriptPath);
      } catch {
        return ineligible('snapshot-unreadable');
      }
      if (invalidated()) return null;
      if (!stat.isFile() || identityChanged(stat)) {
        return ineligible('snapshot-replaced');
      }
      if (stat.size < observedSize) return ineligible('snapshot-shrunk');
      if (stat.size === observedSize) {
        await new Promise(resolve => setTimeout(resolve, WORKFLOW_TRANSCRIPT_POLL_MS));
        continue;
      }

      const candidateSize = stat.size;
      correlation = await correlateWorkflowCompletionSnapshot({
        ...snapshot,
        byteSize: candidateSize,
        prefixByteSize: snapshot.byteSize,
        prefixHash,
      });
      if (invalidated()) return null;
      ...
      let after;
      try {
        after = await fs.promises.stat(snapshot.transcriptPath);
      } catch {
        return ineligible('snapshot-unreadable');
      }
      if (invalidated()) return null;
      if (!after.isFile() || identityChanged(after)) {
        return ineligible('snapshot-replaced');
      }
      if (after.size < candidateSize) return ineligible('snapshot-shrunk');
      observedSize = candidateSize;
      if (after.size > candidateSize) continue;
      if (correlation.eligible) return verifyEligible(correlation);
      if (
        correlation.reason !== 'terminal-not-last-mainline'
        && correlation.reason !== 'partial-line'
      ) {
        return correlation;
      }
    }
    return ineligible('transcript-settle-timeout');
  }

  _beginWorkflowCompletionDecision(snapshot) {
    if (!snapshot) {
      this._logEvent('workflow-completion-ineligible', { reason: 'snapshot-missing' });
      return;
    }
    const generation = this._deliveryGeneration;
    const decisionKey = crypto.createHash('sha256')
      .update(
        `${snapshot.transcriptPath}\0${snapshot.sessionId || ''}\0`
        + `${snapshot.workHookSeq}\0${normalizeVisibleText(snapshot.finalText)}`,
        'utf8',
      )
      .digest('hex');
    const now = Date.now();
    for (const [key, expiry] of this._workflowDecisionKeys) {
      if (expiry <= now) this._workflowDecisionKeys.delete(key);
    }
    if (this._activeWorkflowDecisions.has(decisionKey) || this._workflowDecisionKeys.has(decisionKey)) return;

    this._activeWorkflowDecisions.add(decisionKey);
    this._logEvent('workflow-completion-decision-started', {
      transcript_bytes: snapshot.byteSize,
      final_len: snapshot.finalText.length,
    });

    Promise.resolve()
      .then(() => this._correlateSettledWorkflowSnapshot(snapshot, generation))
      .then(async correlation => {
        if (!correlation || generation !== this._deliveryGeneration || this.closed) return;
        if ((this._workHookSeq || 0) !== snapshot.workHookSeq) {
          this._logEvent('workflow-completion-ineligible', { reason: 'intervening-work' });
          return;
        }
        if (!correlation.eligible) {
          this._logEvent('workflow-completion-ineligible', { reason: correlation.reason });
          return;
        }
        if (correlation.deliveredFinal) {
          this._logEvent('workflow-completion-suppressed', { reason: 'direct-final' });
          return;
        }
        if (correlation.visibleHashes.includes(correlation.finalHash)) {
          this._logEvent('workflow-completion-suppressed', { reason: 'visible-text-match' });
          return;
        }

        for (const pending of correlation.unresolvedAttempts) {
          this._logEvent('workflow-completion-deferred', {
            attempt_id_hash: crypto.createHash('sha256').update(pending.attemptId, 'utf8').digest('hex'),
          });
          const outcome = await this._readDeliveryAttempt(
            pending.attemptId,
            pending.argumentHash,
            generation,
          );
          if (generation !== this._deliveryGeneration || this.closed) return;
          if ((this._workHookSeq || 0) !== snapshot.workHookSeq) {
            this._logEvent('workflow-completion-ineligible', { reason: 'intervening-work' });
            return;
          }
          if (outcome.outcome === 'missing' || outcome.outcome === 'mismatch' || outcome.outcome === 'invalidated') {
            this._logEvent('workflow-completion-ineligible', { reason: `attempt-${outcome.outcome}` });
            return;
          }
          if (
            outcome.outcome === 'sent'
            && (pending.finalReply || pending.visibleHash === correlation.finalHash)
          ) {
            this._logEvent('workflow-completion-suppressed', { reason: 'late-direct-success' });
            return;
          }
        }

        // A new consumer turn makes Stop ownership ambiguous. The fallback
        // covers only the non-overlapping autonomous cycle.
        if (this.pendingTurns.size !== 0) {
          this._logEvent('workflow-completion-ineligible', { reason: 'overlapping-turn' });
          return;
        }
        if ((this._workHookSeq || 0) !== snapshot.workHookSeq) {
          this._logEvent('workflow-completion-ineligible', { reason: 'intervening-work' });
        ...
        this._logEvent('workflow-completion-fallback', {
          final_len: snapshot.finalText.length,
          receipt_count: correlation.visibleHashes.length + correlation.unresolvedAttempts.length,
        });
      })
      .catch(err => {
        this._logEvent('workflow-completion-ineligible', {
      ...
      .finally(() => {
        ...
        this._activeWorkflowDecisions.delete(decisionKey);
        this._workflowDecisionKeys.set(decisionKey, Date.now() + this.contentDedupWindowMs);
        while (this._workflowDecisionKeys.size > RECENT_TOOL_CALL_LIMIT) {
          this._workflowDecisionKeys.delete(this._workflowDecisionKeys.keys().next().value);
        }
        this.emit('delivery-work-settled');
      });
  }
  ...
  async _dispatchToolCall(msg) {
    const args = msg.args || {};
    const correlatedPending = args.turn_id
      ? this.pendingTurns.get(args.turn_id)
      : (this.pendingTurns.size === 1
        ? this.pendingTurns.values().next().value
        : null);
    if (msg.name === 'reply' || msg.name === 'stream' || msg.name === 'edit_message') {
      if (correlatedPending) correlatedPending.outputAttempted = true;
      else this._cleanRestartEvidenceLost = true;
    }
    if (this._cleanRestartOutputFenced) {
      this._cleanRestartEvidenceLost = true;
      if (msg.name === 'ask') {
        this._writeToBridge({
          kind: 'question_answer',
          tool_call_id: msg.tool_call_id,
          result: { cancelled: true, error: 'process retiring' },
        });
      } else {
        this._writeToBridge({
          kind: 'tool_ack',
          tool_call_id: msg.tool_call_id,
          ok: false,
          error: 'process retiring',
          attempt_id: msg.tool_call_id,
        });
      }
      return;
    }
    ...
    // A live-preview snapshot. Handled BEFORE everything below because it must
    // not participate in any of it: not the delivery caches (a preview is not a
    // delivery), not the reply token bucket (a snapshot burst must not starve
    // the reply that ends the turn), and above all not `_noteActivity` — that
    // cancels an in-flight stop-grace, so a trailing snapshot would de-finalize
    // a turn that is already resolving.
    if (msg.name === 'stream') {
      this._handleStreamCall(msg, args);
      return;
    }
    ...
    if (msg.tool_call_id && this.recentToolCallIds.has(msg.tool_call_id)) {
      const prior = this.recentToolCallResults.get(msg.tool_call_id);
      const argumentHash = hashDeliveryArguments(msg.name, args);
      if (!prior || !argumentHash || prior.argumentHash !== argumentHash) {
        this._writeToBridge({
          kind: 'tool_ack',
          tool_call_id: msg.tool_call_id,
          ok: false,
          error: 'duplicate attempt arguments mismatch',
          attempt_id: msg.tool_call_id,
        });
        return;
      }
    ...
    let replyDedupKey = null;
    if (msg.name === 'reply' && typeof args.text === 'string' && args.chat_id != null) {
      replyDedupKey = this._buildContentDedupKey(args);
      const dedupKey = replyDedupKey;
      ...
      if (inflight) {
        ...
        const joinedAttempt = this._registerDeliveryAttempt(msg.tool_call_id, dedupKey);
        let prior;
        try {
          prior = await inflight.promise;
          this._settleDeliveryAttempt(joinedAttempt, prior?.ok ? 'sent' : 'failed');
        } catch (err) {
          this._settleDeliveryAttempt(joinedAttempt, 'failed');
          prior = { ok: false, error: err?.message || 'in-flight dispatch failed' };
        }
        this._writeToBridge({
          ...
          attempt_id: msg.tool_call_id,
          delivery: prior?.ok ? 'replayed' : undefined,
          replay_of: inflight.attemptId,
        });
        return;
      }
    }
    ...
    // Resolve ownership once so dispatch, quoting, and pending-turn bookkeeping
    // cannot disagree. A folded input can own the model's echoed turn_id while
    // the one live primary owns the combined answer; consumed_turn_ids is the
    // explicit proof that connects those two identities.
    const replyAttribution = this._resolveReplyAttribution(
      args.turn_id,
      args.consumed_turn_ids,
    );

    // Resolve the reply's originating message so the dispatcher has a target
    // for reactions and reply-quoting. Anything unattributable stays null; it
    // must never react to or quote an unrelated message.
    //
    ...
    const sourceEntry = replyAttribution.sourceEntry;
    ...
    const argumentHash = replyDedupKey || hashDeliveryArguments(msg.name, args);
    if ((msg.name === 'reply' || msg.name === 'edit_message') && !argumentHash) {
      this._writeToBridge({
        kind: 'tool_ack',
        tool_call_id: msg.tool_call_id,
        ok: false,
        error: 'invalid delivery arguments',
        attempt_id: msg.tool_call_id,
      });
      return;
    }
    const attempt = argumentHash
      ? this._registerDeliveryAttempt(msg.tool_call_id, argumentHash)
      : null;
    const trackedReplyDeliveries = msg.name === 'reply'
      ? this._beginReplyDeliveries([
          replyAttribution.effectiveTurnId,
          ...(Array.isArray(args.consumed_turn_ids) ? args.consumed_turn_ids : []),
        ])
      : [];
    const dispatchPromise = Promise.resolve().then(() => this.toolDispatcher({
        sessionKey: this.sessionKey,
        chatId: this.chatId,
        threadId: this.threadId,
        toolName: msg.name,
        text: args.text,
        // The turn this call names. A consumer holding per-turn state needs it
        // to refuse a late reply from an earlier turn — without it, a delayed
        // reply lands on whatever turn the session is holding NOW.
        turnId: replyAttribution.effectiveTurnId,
    ...
    // Mark this reply's immutable delivery arguments in-flight before awaiting
    // the dispatcher so a retry can join the exact original attempt.
    let inflightKey = null;
    if (replyDedupKey) {
      inflightKey = replyDedupKey;
      this._inFlightDispatches.set(inflightKey, {
        promise: dispatchPromise,
        attemptId: msg.tool_call_id,
      });
    }
    ...
    try {
      ...
      if (attempt) this._settleDeliveryAttempt(attempt, result?.ok ? 'sent' : 'failed');
    } catch (err) {
      if (attempt) this._settleDeliveryAttempt(attempt, 'failed');
      this._finishReplyDeliveries(trackedReplyDeliveries);
      this._writeToBridge({
        kind: 'tool_ack',
        tool_call_id: msg.tool_call_id,
        ok: false,
      ...
      if (
        inflightKey
        && this._inFlightDispatches.get(inflightKey)?.attemptId === msg.tool_call_id
      ) {
    ...
    this._writeToBridge({
      ...
      attempt_id: msg.tool_call_id,
      delivery: result?.ok ? 'sent' : undefined,
    });
    ...
    if (result?.ok && msg.tool_call_id) {
      ...
      this.recentToolCallResults.set(msg.tool_call_id, {
    ...
    if (result?.ok && msg.name === 'reply' && typeof args.text === 'string' && args.chat_id != null) {
      ...
      this.recentContentHashes.set(dedupKey, {
        expiry: Date.now() + this.contentDedupWindowMs,
        message_id: result.message_id ?? null,
        attempt_id: msg.tool_call_id,
      });
    }

    if (result?.ok && msg.name === 'reply') {
      this._recordConsumedReplyDelivery(args.consumed_turn_ids, args.text);
    }
    ...
    if (msg.name === 'reply' && result?.ok && typeof args.text === 'string' && args.text.length > 0) {
      const recorded = this._recordReplyForPendingTurn(
        args.text,
        replyAttribution.effectiveTurnId,
        args.interim === true,
      );
      if (recorded && replyAttribution.foldSource) {
        this._logEvent('cli-fold-reply-attributed', {
          echoed_turn_id: replyAttribution.echoedTurnId,
          effective_turn_id: replyAttribution.effectiveTurnId,
          source: replyAttribution.foldSource,
          interim: args.interim === true,
        });
      }
    }
    this._finishReplyDeliveries(trackedReplyDeliveries);
  ...
  /**
   * A `stream` tool call: a snapshot of the answer the agent is still writing.
   * Forwarded to the consumer as `stream-chunk` — the same event the SDK
   * backend emits per token batch — so the consumer's existing live-preview
   * wiring picks it up unchanged.
   *
   * Ack policy: EVERY outcome acks {ok:true}, including refusals. A false ack
   * becomes an MCP isError, and claude retries on isError — so a rejected
   * snapshot is both free and infinitely repeatable, and the retry loop would
   * cost the turn its real answer. The reason a snapshot was refused is the
   * operator's business, not the agent's: it goes to telemetry only, never to
   * the log (a loop would flood it) and never back over the wire.
   */
  _handleStreamCall(msg, args) {
    const ack = () => this._writeToBridge({
      kind: 'tool_ack',
      tool_call_id: msg.tool_call_id,
      ok: true,
      attempt_id: msg.tool_call_id,
    ...
    const refuse = (reason) => {
      this._logEvent('stream-refused', { reason, turn_id: args?.turn_id ?? null });
      ack();
    };

    // Defense in depth against the env gate. The bridge decides what to
    // REGISTER from its environment, which is not fully ours to control; the
    // daemon decides what to HONOR, which is. A stream call arriving at a
    // process whose consumer never opted in means the bridge was enabled by
    // something other than us — drop it rather than emit stream-chunk at a
    // consumer with nothing wired to receive it.
    if (!this.streamToolEnabled) {
      refuse('tool-not-enabled');
      return;
    }

    // Same chat_id gate every tool call gets — no cross-chat preview injection.
    if (this.chatId != null && String(args.chat_id) !== String(this.chatId)) {
      refuse('chat-id-mismatch');
      return;
    }
    // A blank snapshot would wipe the preview bubble for no reason.
    ...
    if (!turnId) {
      refuse('missing-turn-id');
      return;
    }

    // Attribution: only the turn that owns this turn_id may drive its preview.
    // A foreign cycle (/compact, wakeup, a steered injection) or an already-
    // closed turn has no queue entry; drop it. Falling back to the head would
    // paint one user's preview with another cycle's text. The head check is the
    // same rule from the other side: the consumer routes `stream-chunk` to the
    // head turn's preview, so a snapshot for a queued (non-head) turn would
    // land on the wrong bubble.
    const entry = this.pendingQueue.find(e => e.turnId === turnId);
    if (!entry || this.pendingQueue[0] !== entry) {
      // `reason` matters to the operator: 'not-head' means a queued follow-up
      // turn streamed while an earlier turn held the preview — correct (its
      // snapshots would paint the wrong bubble) but it silently means NO live
      // preview for that turn, and only this field makes that visible.
      this._logEvent('stream-unattributed', {
        turn_id: turnId,
        len: args.text.length,
        reason: entry ? 'not-head' : 'no-pending-turn',
      });
      ack();
      return;
    }

    // Anti-runaway bound, counted ONLY for the attributed turn and stored on
    // its queue entry. Counting before attribution — or in any structure keyed
    // by the caller-supplied turn_id — hands the runaway its own escape: a
    // burst of invented turn_ids drives the live turn's count out of a bounded
    // map and the cap resets forever. The entry is created at dispatch and
    // dropped at settlement, so the count lives exactly as long as the turn it
    // bounds, and nothing outside that turn can touch it.
    entry._streamCalls = (entry._streamCalls || 0) + 1;
    if (entry._streamCalls > STREAM_CALLS_PER_TURN_CAP) {
      // Two events per turn, not one per dropped call: a runaway would write
      // thousands of rows describing the same runaway, but a single event can't
      // tell a chatty turn that brushed the cap from one looping without end.
      // The second, at twice the cap, is what distinguishes them in the soak.
      if (entry._streamCalls === STREAM_CALLS_PER_TURN_CAP + 1
        || entry._streamCalls === STREAM_CALLS_PER_TURN_CAP * 2) {
        this._logEvent('stream-cap-exceeded', {
          turn_id: turnId,
          cap: STREAM_CALLS_PER_TURN_CAP,
          calls: entry._streamCalls,
        });
      }
      ack();
      return;
    ...
    // Composing IS work, so push the idle ceiling out — but only that (the full
    // activity ladder would cancel a stop-grace; see the branch comment above).
    // Placed AFTER attribution and the cap: the idle ceiling is the backstop for
    // a wedged turn, so a snapshot that was dropped — foreign, or past the cap —
    // must not be able to hold that backstop open indefinitely.
    this._resetIdleCeilings();

    // Segmented fresh-spawn vs resumed: a resumed conversation carries
    // in-context evidence of the habit, so adoption there says nothing about
    // whether the prompt alone teaches it. The soak needs the two apart.
    this._logEvent('stream-seen', {
      turn_id: turnId,
      len: args.text.length,
      resumed: this._resumedSession === true,
    });
    this.emit('stream-chunk', args.text);
    ack();
  }

  /**
   * Resolve the turn and ledger entry that own a reply without changing the
   * model-supplied arguments. Fold remapping requires both an echoed fold id
   * and exactly one live pending id in consumed_turn_ids; every other shape
   * preserves the existing late/unknown reply behavior.
   */
  _resolveReplyAttribution(replyTurnId, consumedTurnIds) {
    const echoedTurnId = typeof replyTurnId === 'string' ? replyTurnId : null;
    let effectiveTurnId = echoedTurnId;
    let sourceEntry = echoedTurnId ? (this.inputLedger.get(echoedTurnId) || null) : null;
    let foldSource = null;

    if (sourceEntry
        && !this.pendingTurns.has(echoedTurnId)
        && (sourceEntry.source === 'autosteer' || sourceEntry.source === 'edit-fold')
        && Array.isArray(consumedTurnIds)
        && consumedTurnIds.includes(echoedTurnId)) {
      const pendingIds = new Set(
        consumedTurnIds.filter(id => typeof id === 'string' && this.pendingTurns.has(id)),
      );
      if (pendingIds.size === 1) {
        [effectiveTurnId] = pendingIds;
        foldSource = sourceEntry.source;
        sourceEntry = this.inputLedger.get(effectiveTurnId) || null;
      }
    }

    if (!sourceEntry && this.pendingTurns.size === 1) {
      const [[onlyTurnId]] = this.pendingTurns;
      sourceEntry = this.inputLedger.get(onlyTurnId) || null;
    }

    return { echoedTurnId, effectiveTurnId, sourceEntry, foldSource };
  }
  ...
  /** A reply carried consumed_turn_ids — acknowledge every known input id. */
  ...
  /** Record text only after the dispatcher confirms that the consuming reply arrived. */
  _recordConsumedReplyDelivery(ids, consumingText) {
    if (!Array.isArray(ids) || typeof consumingText !== 'string') return;
    for (const id of new Set(ids)) {
      const pending = this.pendingTurns.get(id);
      if (pending && consumingText.length > (pending._consumedByText?.length || 0)) {
        pending._consumedByText = consumingText;
      }
    }
  }

  /** Keep turn finalization behind the outcome of a consuming reply dispatch. */
  _beginReplyDeliveries(ids) {
    if (!Array.isArray(ids)) return [];
    const tracked = [];
    for (const turnId of new Set(ids)) {
      const pending = this.pendingTurns.get(turnId);
      if (!pending) continue;
      pending._replyDeliveriesInFlight = (pending._replyDeliveriesInFlight || 0) + 1;
      tracked.push({ turnId, pending });
    }
    return tracked;
  }

  _finishReplyDeliveries(tracked) {
    for (const { turnId, pending } of tracked) {
      pending._replyDeliveriesInFlight = Math.max(
        0,
        (pending._replyDeliveriesInFlight || 0) - 1,
      );
      if (pending._replyDeliveriesInFlight > 0 || this.pendingTurns.get(turnId) !== pending) {
        continue;
      }
      if (pending._finalizeAfterReplyDelivery) {
        pending._finalizeAfterReplyDelivery = false;
        pending._timeoutAfterReplyDelivery = null;
        this._finalizeTurn(turnId);
      } else if (pending._timeoutAfterReplyDelivery) {
        const deferred = pending._timeoutAfterReplyDelivery;
        pending._timeoutAfterReplyDelivery = null;
        pending._fireTimeout?.(deferred.reason, deferred.probeResult);
      }
    }
  }
  ...
  /**
   * Push every pending turn's idle ceiling out. The narrow half of
   * `_noteActivity` — proof the agent is working, with none of the finalizer
   * consequences (stop-grace cancellation, quiet-window re-arm). Callers that
   * only know "still working" and must not touch turn resolution use this.
   */
  _resetIdleCeilings() {
    for (const [, pending] of this.pendingTurns) {
      if (!pending.hardTimer) continue;
  ...
  _finalizeTurn(turnId) {
    const pending = this.pendingTurns.get(turnId);
    if (!pending) return;
    if ((pending._replyDeliveriesInFlight || 0) > 0) {
      pending._finalizeAfterReplyDelivery = true;
      return;
    }
  ...
  async send(prompt, opts = {}) {
    if (this._bridgeDisconnecting) {
      throw await this._bridgeDisconnectErrorPromise;
    }
    if (this.closed) {
    ...
    // Hand the turn's id to the caller. A consumer holding per-turn state (a
    // live preview, say) otherwise has no way to tell whether a tool call that
    // names a turn_id belongs to the turn it is holding — and the alternative,
    // reading pendingQueue, is reaching into our internals. A callback rather
    // than a property write on the caller's object: the consumer decides where
    // the id lands, and we never mutate something we were merely handed.
    // Fired before the first await, so it is set by the time any tool call can
    // arrive. A throwing consumer must not take the turn down with it.
    if (typeof queueEntry.context.onTurnId === 'function') {
      try { queueEntry.context.onTurnId(turnId); } catch (err) {
        this.logger.warn?.(`[${this.label}] cli: onTurnId consumer threw: ${err.message}`);
      }
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      const fireTimeout = (reason, probeResult = null) => {
        ...
        const pending = this.pendingTurns.get(turnId);
        if ((pending._replyDeliveriesInFlight || 0) > 0) {
          pending._timeoutAfterReplyDelivery = { reason, probeResult };
          return;
        }
  ...
  hasPendingDeliveryWork() {
    if (this._activeWorkflowDecisions.size > 0) return true;
    for (const entry of this._deliveryAttempts.values()) {
  ...
  wouldReloadFor(spawnContext) {
    return this.reloadReasonsFor(spawnContext).length > 0;
  }

  /**
   * Which spawn-time dimensions have drifted — the drift check itself, with the
   * answer named so telemetry can tell a rendering toggle from a /model change.
   * The display hint is spawn-time too (it is woven into the appended system
   * prompt), so a consumer that varies it per chat needs the same treatment as
   * model/effort. A spawn context that carries no hint at all is a consumer on
   * an older build: `undefined` must not read as "drifted to empty", which would
   * respawn every session on every message.
   * @returns {string[]} empty when nothing drifted or a reload is not possible
   */
  reloadReasonsFor(spawnContext) {
    if (this.inFlight || this.closed) return [];
    const reasons = [];
    if (this._resolveModel(spawnContext) !== this.model) reasons.push('model');
    if (this._resolveEffort(spawnContext) !== this.effort) reasons.push('effort');
    if (
      typeof spawnContext?.displayHint === 'string'
      && spawnContext.displayHint !== this.displayHint
    ) reasons.push('display-hint');
    return reasons;
  }
  ...
  _classifyCleanRestart(deliveryEvidence) {
    if (this.pendingTurns.size === 0) return ['no-active-turn', null];
    if (this.pendingTurns.size !== 1) return ['multiple-active-turns', null];
    const [[turnId, pending]] = this.pendingTurns;
    const queueEntries = this.pendingQueue.filter(entry => entry.turnId === turnId);
    const ledger = this.inputLedger.get(turnId);
    const sourceMsgId = queueEntries[0]?.context?.sourceMsgId ?? ledger?.msgId ?? null;
    if (
      queueEntries.length !== 1
      || ledger?.source !== 'primary'
      || sourceMsgId == null
      || ledger.msgId == null
      || String(sourceMsgId) !== String(ledger.msgId)
    ) {
      return ['uncorrelated-source', null];
    }
    if (
      !deliveryEvidence
      || typeof deliveryEvidence !== 'object'
      || Array.isArray(deliveryEvidence)
      || typeof deliveryEvidence.outputAttempted !== 'boolean'
      || typeof deliveryEvidence.pending !== 'number'
      || !Number.isInteger(deliveryEvidence.pending)
      || deliveryEvidence.pending < 0
      || deliveryEvidence.fenced !== true
    ) {
      return ['delivery-ambiguous', sourceMsgId];
    }
    if (pending.outputAttempted === true || pending.replies?.length > 0) {
      return ['prior-output', sourceMsgId];
    }
    if (deliveryEvidence.outputAttempted) {
      return ['prior-output', sourceMsgId];
    }
    if (
      deliveryEvidence.pending > 0
      || deliveryEvidence.ambiguous === true
      || deliveryEvidence.safe === false
    ) {
      return ['delivery-ambiguous', sourceMsgId];
    }
    if (pending.activeToolIds?.size > 0) return ['unresolved-tool', sourceMsgId];
    if (this._openQuestions.size > 0) return ['unresolved-question', sourceMsgId];
    if (this._openApprovals.size > 0) return ['unresolved-approval', sourceMsgId];
    if (this._pendingSubagentStarts.length > 0) return ['active-subagent', sourceMsgId];
    if (ledger.state !== 'seen') return ['hook-correlation-missing', sourceMsgId];
    if (this.hasPendingDeliveryWork()) return ['delivery-ambiguous', sourceMsgId];
    const nonterminal = [...this.inputLedger.entries()].filter(([, entry]) => (
      entry.state === 'written'
      || entry.state === 'seen'
      || entry.state === 'fold-suspected'
    ));
    if (
      nonterminal.length !== 1
      || nonterminal[0][0] !== turnId
      || this.pendingQueue.length !== 1
    ) {
      return ['other-action-owner', sourceMsgId];
    }
    if (this._cleanRestartEvidenceLost) return ['bridge-loss', sourceMsgId];
    return ['eligible', sourceMsgId];
  }

  _cleanRestartSourceMsgId() {
    if (this.pendingTurns.size !== 1) return null;
    const [[turnId]] = this.pendingTurns;
    const queueEntries = this.pendingQueue.filter(entry => entry.turnId === turnId);
    const ledger = this.inputLedger.get(turnId);
    const sourceMsgId = queueEntries[0]?.context?.sourceMsgId ?? ledger?.msgId ?? null;
    if (
  ...
  async retireForCleanRestart({ getDeliveryEvidence } = {}) {
    if (typeof getDeliveryEvidence !== 'function') {
      throw new TypeError(
        'CliProcess.retireForCleanRestart: getDeliveryEvidence function required',
      );
    }
    if (this._cleanRestartRetirement) return this._cleanRestartRetirement;
    this._cleanRestartOutputFenced = true;
    this._cleanRestartRetiring = true;
    this._cleanRestartRetirement = (async () => {
      const failures = [];
      let deliveryEvidence = null;
      try {
        deliveryEvidence = await getDeliveryEvidence(
          this.sessionKey,
          this._cleanRestartSourceMsgId(),
        );
      } catch (error) {
        failures.push(error);
      }
      if (this.tmuxSession) {
        try {
          await this.runner.killSession(this.tmuxSession, { strict: true });
      ...
      if (!this._hookTail || typeof this._hookTail.drain !== 'function') {
        failures.push(new Error('hook tail unavailable'));
      } else {
        try {
          await this._hookTail.drain();
        } catch (error) {
          failures.push(error);
        }
        try { this._hookTail.close(); } catch (error) { failures.push(error); }
        this._hookTail = null;
      }

      const [reason, sourceMsgId] = this._classifyCleanRestart(deliveryEvidence);
      try {
        await this._doKill('clean-restart');
      } catch (error) {
        failures.push(error);
      }
      if (failures.length > 0) {
        ...
        throw Object.assign(
          new Error(`clean restart retirement failed: ${cause.message}`, { cause }),
          {
    ...
    return this._cleanRestartRetirement;
  }
  ...
  _handleHookEvent(ev) {
    ...
    if (ev.type === 'Stop' || orphanSubagentStop) {
      ...
      // The live-preview tool is the ONE tool call that must not run the full
      // activity ladder. The Pre/PostToolUse matcher is `.*`, so the agent's own
      // `stream` calls arrive here like any other tool — and _noteActivity
      // CANCELS an in-flight stop-grace once hooks are live. A trailing snapshot
      // would then de-finalize a turn that was already resolving, which is
      // exactly what the socket-side handler refuses to do. Its idle-ceiling
      // reset happens there instead, after attribution.
      // (_lastHookEventAt and _workHookSeq above are deliberately left alone:
      // neither touches stop-grace, and suppressing them would quietly change
      // the other finalizer rungs.)
      const isStreamToolHook = (ev.type === 'PreToolUse' || ev.type === 'PostToolUse')
        && ev.toolName === this._streamToolHookName;
      if (!isStreamToolHook) this._noteActivity(`hook:${ev.type}`);
    }
    ...
    switch (ev.type) {
      ...
      case 'PreToolUse': {
        const activePending = this.pendingTurns.size === 1
          ? this.pendingTurns.values().next().value
          : null;
        if (activePending && ev.toolUseId) {
          (activePending.activeToolIds ||= new Set()).add(ev.toolUseId);
      ...
      case 'PostToolUse':
        for (const pending of this.pendingTurns.values()) {
          pending.activeToolIds?.delete(ev.toolUseId);
        }
      ...
      case 'Stop': {
        ...
        const preStopPendingCount = this.pendingTurns.size;
        const workflowSnapshot = preStopPendingCount === 0 && ev.stopHookActive !== true
          ? this._captureWorkflowCompletionSnapshot(ev)
          : null;
        const info = {
        ...
        if (preStopPendingCount === 0 && this.pendingTurns.size === 0) {
          this._beginWorkflowCompletionDecision(workflowSnapshot);
        }
   ...
   */
  async _handleBridgeDisconnected(reason = 'socket-close') {
    if (this._cleanRestartRetiring) {
      this.bridgeReady = false;
      this.mcpReady = false;
      return;
    }
    ...
    this.mcpReady = false;
    this._stopStartupPingLoop();
    if (this.closed || this._bridgeDisconnecting) return;
    this._bridgeDisconnecting = true;
    this._bridgeDisconnectErrorPromise = this._bridgeDisconnectError();
    const disconnectError = await this._bridgeDisconnectErrorPromise;
    this._invalidatePendingDeliveryWork();
    ...
    for (const [, pending] of this.pendingTurns) {
      ...
      if (pending._onStop) this.off('stop-hook', pending._onStop);
      try { pending.reject(disconnectError); } catch {}
    }
    ...
    this.emit('bridge-disconnected', { code: disconnectError.code });
    this._logEvent('bridge-disconnected', { reason, code: disconnectError.code });
  }

  async _bridgeDisconnectError() {
    if (!this.sessionLauncher) {
      return Object.assign(new Error('bridge disconnected'), { code: 'BRIDGE_DISCONNECTED' });
    }

    let processAlive = false;
    if (this.tmuxSession && typeof this.runner?.sessionProcessAlive === 'function') {
      let timeout;
      try {
        processAlive = await Promise.race([
          this.runner.sessionProcessAlive(this.tmuxSession),
          new Promise(resolve => {
        ...
        this.logger.warn?.(`[${this.label}] channels: could not verify contained process after bridge disconnect: ${err.message}`);
      } finally {
    ...
    return Object.assign(new Error('contained session process was lost'), { code: 'SESSION_PROCESS_LOST' });
  }

  async _doKill(reason) {
    this.closed = true;
    this.inFlight = false;
    this._cancelPendingBridgeHandshake('KILLED', `session killed: ${reason}`);
    this._invalidatePendingDeliveryWork();
    this._stopStartupPingLoop();
  ...
  async resetSession({ reason = 'reset' } = {}) {
    this._cancelPendingBridgeHandshake('RESET', `session reset: ${reason}`);
    this._invalidatePendingDeliveryWork();
    let drained = 0;
  ...
  async respondToPermission(requestId, behavior) {
    ...
    this._openApprovals.delete(requestId);
  ...
  _startStartupPingLoop() {
    if (this.startupPingTimer) return;
    this._writeToBridge({ kind: 'ping' });
    this.startupPingTimer = setInterval(() => {
      this._writeToBridge({ kind: 'ping' });
    }, PING_INTERVAL_MS);
    this.startupPingTimer.unref?.();
  }

  _stopStartupPingLoop() {
    if (!this.startupPingTimer) return;
    clearInterval(this.startupPingTimer);
    this.startupPingTimer = null;
  }
