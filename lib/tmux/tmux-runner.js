...
// A tmux socket name becomes a bare CLI argument. execFile means this is not a
// shell-injection vector, but a value starting with '-' would be parsed by tmux
// as another server flag, and a path separator would silently select a
// different socket directory. Same reasoning as the paneWidth guard in spawn().
const SOCKET_NAME_RE = /^[A-Za-z0-9][A-Za-z0-9_.-]{0,63}$/;
 ...
 * @param {string} [opts.socketName] — run against a dedicated tmux server
 *   (`tmux -L <name>`) instead of the shared default socket.
 */
function createTmuxRunner({
  ...
  if (socketName != null && !SOCKET_NAME_RE.test(socketName)) {
    throw new TypeError(
      `socketName must match ${SOCKET_NAME_RE} (got ${JSON.stringify(socketName)})`,
    );
  }
  const runnerPrefix = sessionPrefix;
  // Server-scope flags, in tmux's required position: before the command word.
  //
  // `-L <name>` gives this app its own tmux server. Without it orchestra shares
  // the DEFAULT socket with the operator's interactive sessions, so one
  // `tmux kill-server` — from a unit's ExecStop or from a human at a prompt —
  // reaches every bot, water and the admin session at once.
  //
  // Blast-radius control, not a privilege boundary: every server here runs under
  // the same uid, so any process that knows the name can still run
  // `tmux -L <name>` against it.
  const serverArgs = [
    ...(socketName ? ['-L', socketName] : []),
    ...(requireExistingServer ? ['-N'] : []),
  ];
  const runTmux = (args, opts) => runFn('tmux', serverArgs.length ? [...serverArgs, ...args] : args, opts);
  async function spawn({
    ...
    try {
      await runTmux(['resize-window', '-t', name, '-x', String(paneWidth)]);
    } catch (err) {
  ...
  async function pasteText(name, text) {
    ...
    await runTmux(['set-buffer', '-b', bufName, oneLine]);
    try {
      // -d (delete after) so the buffer doesn't accumulate.
      await runTmux(['paste-buffer', '-t', name, '-b', bufName, '-d']);
    } catch (err) {
      // Best-effort buffer cleanup if paste fails.
      await runTmux(['delete-buffer', '-b', bufName]).catch(() => {});
      throw err;
    }
  ...
  async function sessionProcessAlive(name) {
    try {
      ...
      return stdout
        .trim()
        .split('\n')
        .some(line => /^0 [1-9][0-9]*$/.test(line.trim()));
    } catch {
      return false;
    }
  }
  ...
  /**
   * Kill a tmux session. Best-effort by default (teardown paths race normal
   * exits and must not throw). Pass `{ strict: true }` when the caller reports
   * success or failure to someone: silently swallowing here makes a stale
   * session look reaped, and the next spawn then collides with the name.
   */
  async function killSession(name, { strict = false } = {}) {
    try {
      await runTmux(['kill-session', '-t', name]);
    } catch (err) {
      if (strict) {
        throw Object.assign(new Error(`tmux kill-session failed: ${err.message}`), {
   ...
   *
   * Default is best-effort: an unreachable tmux server reads as an empty host.
   * Pass `{ strict: true }` when the caller must distinguish "no sessions" from
   * "could not ask" — with a dedicated socket and `-N` (never auto-start), an
   * unreachable server means the tmux unit failed to come up, and treating that
   * as "nothing here" hides it.
   */
  async function listPolygramSessions(botName = null, { strict = false } = {}) {
    try {
      ...
      if (strict) {
        throw Object.assign(new Error(`tmux list-sessions failed: ${err.message}`), {
          code: 'TMUX_LIST_FAILED',
          cause: err,
        });
      }
      return [];
