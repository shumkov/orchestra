...
const graceMs = argv[0]?.startsWith(GRACE_FLAG)
  ...
  : Number.NaN;
const launcherFlag = '--session-launcher=';
const sessionLauncher = argv[0]?.startsWith(launcherFlag)
  ? argv.shift().slice(launcherFlag.length)
  : null;
const [binary, ...args] = argv;
if (!binary || !Number.isSafeInteger(graceMs) || graceMs < 0) process.exit(64);

const child = spawn(sessionLauncher ?? binary, sessionLauncher
  ? [binary, ...args]
  : args, {
  cwd: process.cwd(),
  env: process.env,
