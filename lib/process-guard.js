...
const CLAIM_PID_FILE_THROWS_ON_SURVIVING_PREDECESSOR = true;
    ...
    if (!parsed) {
      ...
      if (!alive) {
        ...
        if (isAlive(parsed)) {
          ...
          while (Date.now() - killStart < sigkillWaitMs && isAlive(parsed)) {
            sleepSync(20);
          }
          if (isAlive(parsed)) {
            const error = new Error(
              `prior daemon PID ${parsed} remained alive after SIGKILL`,
            );
            error.code = 'PROCESS_GUARD_PREDECESSOR_ALIVE';
            throw error;
          }
...
module.exports = {
  CLAIM_PID_FILE_THROWS_ON_SURVIVING_PREDECESSOR,
  CODEX_SUPERVISOR_GRACE_MS,
  claimPidFile,
  releasePidFile,
