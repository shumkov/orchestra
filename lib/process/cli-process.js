...
class CliProcess extends Process {
  ...
  async start(opts = {}) {
    ...
    this.sockPath = path.join(os.tmpdir(), `water-${socketToken}.sock`);
    ...
    const tmuxName = `water-${this.botName}-channels-${socketToken.slice(0, 8)}`;
  ...
  async _spawnTmuxClaude({ tmuxName, opts }) {
    ...
    const socketToken = path.basename(this.sockPath, '.sock').replace(/^water-/, '');
    this.mcpConfigPath = path.join(os.tmpdir(), `water-${socketToken}-mcp.json`);
  ...
  async _dispatchToolCall(msg) {
    ...
    if (sourceEntry && !sourceEntry._quoteUsed) {
      // orchestra: message ids may be numeric (Telegram) OR opaque strings
      // (WhatsApp stanza ids). Preserve a non-empty string id as-is; only fall back
      // to numeric coercion for numeric-id transports. A blind Number() dropped
      // WhatsApp quote/reaction targets to null.
