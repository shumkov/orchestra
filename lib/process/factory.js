...
// The SDK backend is CONSUMER-INJECTED (orchestra): polygram supplies its own
// SdkProcess (which pulls the Anthropic SDK), water omits it (cli-only). If a chat
// selects pm:'sdk' and no SdkProcess was injected, we throw a clear error rather than
// silently failing.
class MissingSdkProcess {
  constructor() {
    throw new Error(
      "orchestra: pm:'sdk' selected but no SdkProcess was injected into " +
      "createProcessFactory({ SdkProcess }). Inject it, or use pm:'cli'.",
    );
  }
}
...
function createProcessFactory({
  ...
  SdkProcess = MissingSdkProcess,               // consumer-injected SDK backend class
} = {}) {
