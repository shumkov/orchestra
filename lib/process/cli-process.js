...
class CliProcess extends Process {
   ...
   *
   * Ack policy: once the guards pass the ack is ALWAYS {ok:true}, even when the
   * snapshot is dropped. A preview is cosmetic; telling the agent a snapshot
   * failed would invite retry loops that cost the turn its real answer.
   */
  _handleStreamCall(msg, args) {
    const ack = (ok, error) => this._writeToBridge({
      kind: 'tool_ack',
      tool_call_id: msg.tool_call_id,
      ok,
      ...(error ? { error } : {}),
    ...
    // something other than us — refuse it and say so, rather than emitting
    // stream-chunk at a consumer with nothing wired to receive it.
    if (!this.streamToolEnabled) {
      this._logEvent('stream-tool-not-enabled', { turn_id: args.turn_id ?? null });
      this.logger.warn?.(
        `[${this.label}] channels: stream call rejected — the stream tool is not enabled for this session`,
      );
      ack(false, 'stream tool not enabled for this session');
      return;
    }

    // Same chat_id gate every tool call gets — no cross-chat preview injection.
    if (this.chatId != null && String(args.chat_id) !== String(this.chatId)) {
      ack(false, 'chat_id mismatch');
      return;
    }
    // A blank snapshot would wipe the preview bubble for no reason.
    if (typeof args.text !== 'string' || args.text.trim().length === 0) {
      ack(false, 'stream.text missing or empty');
      return;
    }
    const turnId = typeof args.turn_id === 'string' ? args.turn_id : '';
    if (!turnId) {
      ack(false, 'stream.turn_id missing');
      return;
    }
    ...
    if (entry._streamCalls > STREAM_CALLS_PER_TURN_CAP) {
      // One event per turn, not per dropped call — a runaway would otherwise
      // write thousands of rows describing the same runaway.
      if (entry._streamCalls === STREAM_CALLS_PER_TURN_CAP + 1) {
        this._logEvent('stream-cap-exceeded', { turn_id: turnId, cap: STREAM_CALLS_PER_TURN_CAP });
      }
      ack(true);
      return;
    }
