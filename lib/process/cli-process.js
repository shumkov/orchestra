...
class CliProcess extends Process {
  ...
  async send(prompt, opts = {}) {
    ...
    // Publish the turn's id back on the caller's own context object. A consumer
    // holding per-turn state (a live preview, say) otherwise has no way to tell
    // whether a tool call that names a turn_id belongs to the turn it is holding
    // — and the alternative, reading pendingQueue, is reaching into our
    // internals. Written before the first await so it is set by the time any
    // tool call can arrive.
    queueEntry.context.turnId = turnId;
