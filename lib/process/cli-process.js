...
class CliProcess extends Process {
  ...
  async _spawnTmuxClaude({ tmuxName, opts }) {
    ...
    claudeArgs.push('--append-system-prompt', [
      ...
      '5. Mark the item you are working on RIGHT NOW with a trailing hourglass:',
      '   `- [ ] Проверяю конфиги… ⏳` — at the END of that line, never the start',
      '   (the checkbox column stays aligned). Exactly one item carries it at a',
      '   time; when the item completes it becomes `- [x]` and the ⏳ moves to',
      '   the next one. The user sees at a glance what is happening right now.',
      ...
      '   the ⏳. Its final state is all `- [x]`, zero ⏳. A turn that ends with',
      '   a lingering ⏳ on the checklist is a CONTRACT VIOLATION — the user is',
      ...
      '   remove the ⏳, leave that item unticked, and name the reason ON its',
      '   line, e.g. `- [ ] Деплой — заблокировано: нет доступа к VPS`. The',
      '   lingering ⏳ is the violation; an honestly labelled unfinished item',
      '   is not.',
      '',
