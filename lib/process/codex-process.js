...
} = require('../codex/thread-attachment-settings');
...
const CLIENT_ROOT_ERROR_CODES = new Set([
...
const CLIENT_FAULT_CLASSES = new Set([
  'stderr-limit',
  'transport',
  'protocol',
  'process-exit',
...
function readClientFaultProvenance(error) {
  const seen = new Set();
  let current = error;
  for (let depth = 0; current && depth < 8; depth += 1) {
    ...
    const faultClass = Object.getOwnPropertyDescriptor(
      current,
      'clientFaultClass',
    )?.value;
    if (
      typeof root === 'string'
      && CLIENT_ROOT_ERROR_CODES.has(root)
      && typeof faultClass === 'string'
      && CLIENT_FAULT_CLASSES.has(faultClass)
      && (root !== 'unknown' || faultClass === 'unknown')
    ) {
      return Object.freeze({
        clientRootErrorCode: root,
        clientFaultClass: faultClass,
      });
    }
...
function attachClientFaultProvenance(error, provenance) {
...
function dynamicSettings(source) {
  const parsed = parseCodexAttachmentSettings({
    model: source?.model,
    reasoningEffort: source?.reasoningEffort ?? source?.effort,
  });
...
class CodexProcess extends Process {
  ...
  set state(next) {
    if (
      ...
      && !(
        next === 'Closed'
        && this.containmentCleanupCommitted
      )
    ) return;
    if (
  ...
  constructor({
    ...
    if (hookVerifier !== null && typeof hookVerifier !== 'function') {
      throw new TypeError('CodexProcess: hookVerifier must be a function');
    }
    ...
    this.hookVerifier = hookVerifier;
    ...
    this.provisionalAttachmentModel = null;
    this.attachmentResponseValidated = false;
    this.admittingTurnSettings = null;
    this.activeTurnSettings = null;
    this.lastAcceptedTurnSettings = null;
    ...
    this.containmentErrorCode = null;
    this.containmentTurnId = null;
    this.containmentProvenance = null;
    this.containmentCleanupCommitted = null;
  ...
  _advanceActivityEpoch() {
    if (this.activityEpoch === Number.MAX_SAFE_INTEGER) {
      throw processError(
        'Codex activity epoch is exhausted',
        'CODEX_ACTIVITY_EPOCH_EXHAUSTED',
      );
    }
  ...
  async _start(options) {
    ...
    if (
      options.resumePolicy != null
      && options.resumePolicy !== 'require-interrupted-turn'
    ) {
      throw processError(
        'Unsupported Codex resume policy',
        'CODEX_STRICT_RESUME_INVALID',
      );
    }
    const strictResume = options.resumePolicy === 'require-interrupted-turn';
    if (!strictResume && options.expectedInterruptedTurnId != null) {
      throw processError(
        'Expected interrupted turn requires strict Codex resume',
        'CODEX_STRICT_RESUME_INVALID',
      );
    }
    const expectedInterruptedTurnId = strictResume
      ? boundedOpaqueId(
        options.expectedInterruptedTurnId,
        'expectedInterruptedTurnId',
      )
      : null;
    if (strictResume && !existingSessionId) {
      throw processError(
        'Strict Codex resume requires an existing thread',
        'CODEX_STRICT_RESUME_INVALID',
      );
    }
    ...
    try {
      ...
      if (
        ...
        || (
          this.hookVerifier !== null
          && typeof this.client.verifyHooks !== 'function'
        )
      ) {
        throw new TypeError(
      ...
      // A resumed thread executes hooks exactly as a fresh one does, so trust
      // is proven before either kind of attachment.
      await this._verifyHookTrust();
      this._assertLifecycle(lifecycle);
      ...
      if (strictResume) {
        await this._attestStrictResume(
          result,
          existingSessionId,
          expectedInterruptedTurnId,
          lifecycle,
      ...
      this._assertLifecycle(lifecycle);
      if (strictResume) {
        this._resumeAttestation = Object.freeze({
          namespace: 'codex:app-server',
          sessionId: existingSessionId,
          interruptedTurnId: expectedInterruptedTurnId,
          resumed: true,
          freshFallback: false,
          idle: true,
        });
  ...
  async _verifyHookTrust() {
    if (!this.hookVerifier) return;
    try {
      await this.hookVerifier(this.client, 'trusted');
    } catch (cause) {
      throw processError(
        'Codex hook trust could not be verified',
        cause?.code ?? 'CODEX_HOOK_TRUST_UNVERIFIED',
        { cause },
      );
    }
  }
  ...
  _validateThreadResult(result, expectedThreadId) {
    ...
    this.attachmentResponseValidated = true;
  }

  async _attestStrictResume(
    result,
    expectedThreadId,
    expectedInterruptedTurnId,
    lifecycle,
    ...
    const tail = Array.isArray(thread?.turns) ? thread.turns.at(-1) : null;
    if (
      thread?.id !== expectedThreadId
      || thread.ephemeral !== false
      || thread.status?.type !== 'idle'
      || tail?.id !== expectedInterruptedTurnId
      || tail.status !== 'interrupted'
    ) {
      throw processError(
        'Codex strict resume did not attest the exact interrupted turn',
        'CODEX_STRICT_RESUME_MISMATCH',
      );
    }
    const background = await this.client.request(
      'thread/backgroundTerminals/list',
      { threadId: expectedThreadId, limit: 100 },
      { timeoutMs: this.cleanupTimeoutMs },
    );
    this._assertLifecycle(lifecycle);
    if (background?.count !== 0 || background.nextCursor != null) {
      throw processError(
        'Codex strict resume found unresolved background work',
        'CODEX_STRICT_RESUME_MISMATCH',
      );
    }
  }
  ...
  _validateAndObserveThreadPolicy(
    ...
    const expectedStaticPolicy = attachmentResponse
    ...
    if (attachmentResponse) {
      ...
      try {
        attachmentSettings = parseCodexAttachmentSettings(source);
      } catch (cause) {
        throw processError(
          'Codex thread returned malformed attachment settings',
          'CODEX_PROTOCOL_ERROR',
          { cause },
        );
      }
      ...
      if (
        ...
        && (
          ...
          || collaboration.model !== attachmentSettings.model
          || (
            attachmentSettings.complete
            && collaboration.reasoningEffort != null
            && collaboration.reasoningEffort !== attachmentSettings.effort
          )
        )
      ) {
        throw processError(
      ...
      const accepted = this._completeThreadSettingsCandidates();
      if (attachmentSettings.complete) {
        const observed = modelSettings(
          attachmentSettings.model,
          attachmentSettings.effort,
          'thread settings',
        );
        if (
          accepted.length > 0
      ...
      if (
        accepted.length > 0
        && !accepted.some(
          (candidate) => candidate.model === attachmentSettings.model,
        )
      ) {
        throw processError(
          'Codex thread attachment reported inconsistent dynamic settings',
      ...
      if (accepted.length === 0) {
        this.provisionalAttachmentModel = attachmentSettings.model;
      } else {
        this.provisionalAttachmentModel = null;
      }
      return;
    ...
    if (
      accepted.length === 0
      && this.provisionalAttachmentModel
      && observed.model !== this.provisionalAttachmentModel
    ) {
      throw processError(
        'Codex thread reported unexplained dynamic settings',
        'CODEX_THREAD_SETTINGS_MISMATCH',
    ...
    if (
      ...
      && !(
        this.state === 'AttachingThread'
        && !this.attachmentResponseValidated
      )
    ) {
      throw processError(
        'Codex thread reported unexplained dynamic settings',
  ...
  send(prompt, {
    ...
    const pending = {
      ...
      responseSeen: false,
      startedSeen: false,
      startDeliveryState: 'queued',
      startResponseOutcome: null,
      startCancellationRequested: false,
      startPreparationDisposition: makeDeferred(),
  ...
  _pump() {
    ...
    this.inFlight = true;
    pending.startDeliveryState = 'attesting';
    ...
    try {
      const responseTurnId = await this._withSettingsGate(async () => {
        ...
        try {
          ...
          // A long-lived session must not drift between attachment and its
          // next turn, so trust is re-proven for every turn.
          try {
            await this._verifyHookTrust();
          } catch (cause) {
            cause.deliveryState = 'not-sent';
            throw cause;
          }
          ...
          const result = await this._mutation('turn/start', params, {
            ...
            onPreparationDisposition: (disposition) => {
              pending.startPreparationDisposition.resolve(disposition);
            },
          ...
          this._assertLifecycle(lifecycle);
          this.lastAcceptedTurnSettings = admittedSettings;
  ...
  _resolveStartAsNotStarted(pending) {
    pending.startDeliveryState = 'definitely-not-sent';
    pending.startPreparationDisposition.resolve({ kind: 'not-started' });
    pending.startDisposition.resolve({ kind: 'not-started' });
  }
  ...
  _resolveStartDispositionFromError(pending, error) {
    ...
    const durability = this._durabilityFailure(error);
    if (
      pending.startDeliveryState === 'queued'
      || pending.startDeliveryState === 'attesting'
    ) {
      this._resolveStartAsNotStarted(pending);
      return;
    }
    if (pending.startDeliveryState === 'checkpoint-not-committed') {
      const disposition = {
        kind: 'checkpoint-not-committed',
        cause: durability ?? error,
      };
      pending.startDisposition.resolve(disposition);
      return;
    }
    if (pending.startDeliveryState === 'checkpoint-unknown') {
      const disposition = {
        kind: 'checkpoint-unknown',
        cause: durability ?? error,
      };
      pending.startDisposition.resolve(disposition);
      return;
    ...
    if (
      ...
      if (
        ...
        pending.startDisposition.resolve({ kind: 'checkpoint-committed' });
        return;
      }
      this._resolveStartAsNotStarted(pending);
      return;
  ...
  async _resolveInterruptStartDisposition(
    pending,
    controlDeadline,
    lifecycle,
  ) {
    ...
    const waitMs = Math.max(
      1,
      Math.min(this.turnStartTimeoutMs, controlDeadline - Date.now()),
    );
    if (
      pending.startDeliveryState === 'preparing'
      || pending.startDeliveryState === 'checkpoint-not-committed'
      || pending.startDeliveryState === 'checkpoint-unknown'
    ) {
      try {
        return await timeout(
          pending.startPreparationDisposition.promise,
          waitMs,
          'Codex turn preparation disposition was not observed before interruption',
          'CODEX_TURN_START_TIMEOUT',
        );
      } catch (error) {
        return { kind: 'checkpoint-unknown', cause: error };
      }
    }
    if (pending.startDeliveryState === 'prepared') {
      return { kind: 'checkpoint-committed' };
    }
    if (
      pending.startDeliveryState === 'write-checkpointing'
      || pending.startDeliveryState === 'write-attempted'
      ...
      try {
        const disposition = await timeout(
    ...
    if (pending.startDeliveryState === 'accepted') {
      return { kind: 'accepted', turnId: pending.turnId };
    }
    return { kind: pending.startDeliveryState };
  }
  ...
  async interrupt(reason = 'interrupt') {
    ...
    const interruptionReason = boundedOpaqueId(reason, 'interruption reason');
    this.interruptPromise = this._interrupt(interruptionReason);
    return this.interruptPromise;
  }

  async _interrupt(interruptionReason, controlDeadlineOverride = null) {
    ...
    const pending = this.current;
    let backgroundOwnership = null;
    if (
      priorState === 'BackgroundWorking'
      || priorState === 'BackgroundSettling'
    ) {
      try {
        backgroundOwnership = this._requireBackgroundOwner();
      } catch (error) {
        await this._enterContainment(
    ...
    const controlDeadline = controlDeadlineOverride
      ?? pending?.deadlineAt
      ?? backgroundOwnership?.owner.deadlineAt
      ?? (Date.now() + this.interruptTimeoutMs + this.cleanupTimeoutMs);
    let turnId = this.activeTurnId
      ?? pending?.turnId
      ?? backgroundOwnership?.owner.turnId
      ?? null;
    ...
    this.state = 'Quiescing';
    const lifecycle = this._captureLifecycle();
    this._drainWaiting('INTERRUPTED', interruptionReason);
    ...
    if (pending && priorState === 'StartingTurn') {
      const disposition = await this._resolveInterruptStartDisposition(
        pending,
        controlDeadline,
        lifecycle,
      );
      if (disposition.kind === 'accepted') {
      ...
      } else if (disposition.kind === 'not-started') {
        turnId = null;
        const cancelled = processError(
          'Codex turn start was interrupted before preparation',
          'INTERRUPTED',
        );
        pending.cancelled = true;
        this._rejectPending(pending, cancelled);
        this._finishPending(pending);
      } else if (disposition.kind === 'checkpoint-committed') {
        turnId = null;
        try {
        ...
        this._rejectPending(pending, cancelled);
        this._finishPending(pending);
      } else if (disposition.kind === 'checkpoint-not-committed') {
        const cause = disposition.cause ?? processError(
          'Codex turn preparation was rejected without a committed attempt',
          'CODEX_DURABILITY_FAILED',
        );
        this._blockDurability(cause, pending);
        throw processError(
          'Codex turn preparation was not durably committed',
          'CODEX_DURABILITY_FAILED',
          { cause },
        );
      } else {
    ...
    try {
      ...
      const reuseBackgroundCleanup = (
        ...
        && cleanupProof.deadlineAt === backgroundOwnership?.owner.deadlineAt
      );
  ...
  async _refreshBackgroundState(pending, lifecycle) {
    ...
    if (this.state !== 'Settling' || this.current !== pending) return;
    const probeState = this.state;
    const page = await this.client.request(
      'thread/backgroundTerminals/list',
      { threadId: this.providerSessionId, limit: 100 },
      {
    ...
    this._assertLifecycle(lifecycle);
    if (this.state !== probeState || this.current !== pending) return;
    if (page.count > 0 || page.nextCursor != null) {
      this._bindBackgroundOwnerFromPending(pending);
      this._activateBackgroundOwnership();
      this._armBackgroundWatchdog();
  ...
  _activateBackgroundOwnership() {
    this._requireBackgroundOwner();
  ...
  _backgroundOwnershipError(message) {
    return processError(message, 'CODEX_BACKGROUND_OWNERSHIP_UNKNOWN');
  }

  _ownerFromTerminal(terminal, { requireLive = false } = {}) {
    if (
      !terminal
      || !Number.isSafeInteger(terminal.deadlineAt)
      || terminal.deadlineAt < 1
      || !['completed', 'interrupted', 'failed'].includes(terminal.status)
      || (requireLive && terminal.deadlineAt <= Date.now())
    ) {
      throw this._backgroundOwnershipError(
        'Codex background work had no exact live terminal owner',
      );
    }
    let turnId;
    try {
      turnId = boundedOpaqueId(terminal.turnId, 'background owner turnId');
    } catch {
      throw this._backgroundOwnershipError(
        'Codex background work had no exact live terminal owner',
      );
    ...
    return Object.freeze({
      turnId,
      deadlineAt: terminal.deadlineAt,
    });
  }

  _bindBackgroundOwner(terminal, options = {}) {
    const owner = this._ownerFromTerminal(terminal, options);
    if (this.backgroundOwner) {
      if (
        !Object.isFrozen(this.backgroundOwner)
        || this.backgroundOwner.turnId !== owner.turnId
        || this.backgroundOwner.deadlineAt !== owner.deadlineAt
      ) {
        throw this._backgroundOwnershipError(
          'Codex background owner changed before cleanup',
        );
      }
      return this.backgroundOwner;
    }
    this.backgroundCleanupProof = null;
    this.backgroundOwner = owner;
    return owner;
  }

  _bindBackgroundOwnerFromPending(pending) {
    ...
    if (
      ...
      || terminal.deadlineAt !== pending.deadlineAt
    ) {
      throw this._backgroundOwnershipError(
        'Codex background registry had no exact pending terminal owner',
      );
    }
    return this._bindBackgroundOwner(terminal);
  }

  _captureBackgroundPromotionCandidate() {
  ...
  _promoteBackgroundCandidateToOwner(admission) {
    const candidate = admission?.candidate;
    const terminal = this.lastTerminal;
    if (
      !admission
      || this.activityEpoch !== admission.activityEpoch
      || this.state !== admission.state
      || this.current !== admission.current
      || !candidate
      || candidate.terminal !== terminal
      || candidate.turnId !== terminal?.turnId
      || candidate.status !== terminal?.status
      || candidate.deadlineAt !== terminal?.deadlineAt
    ) {
      throw this._backgroundOwnershipError(
        'Codex background status candidate changed before ownership',
      );
    }
    return this._bindBackgroundOwner(terminal, {
      requireLive: true,
  ...
  _requireBackgroundOwner() {
    ...
    if (
      ...
      || !terminal
      || owner.turnId !== terminal.turnId
      || owner.deadlineAt !== terminal.deadlineAt
    ) {
      throw this._backgroundOwnershipError(
        'Codex background work had missing or mismatched ownership',
      );
    }
    this._ownerFromTerminal(terminal);
    return { owner, terminal };
  }

  _qualificationFenceError() {
    return processError(
      'Codex clean-restart qualification fence changed',
      'CODEX_QUALIFICATION_FENCE',
    );
  }

  _assertQualificationFence(fence, assertLease) {
    if (
      fence.lifecycleEpoch !== this.lifecycleEpoch
      || fence.generationId !== this.generationId
      || fence.activityEpoch !== this.activityEpoch
    ) {
      throw this._qualificationFenceError();
    }
    if (typeof assertLease !== 'function') return;
    try {
      if (assertLease() === false) throw this._qualificationFenceError();
    } catch (error) {
      if (
        error?.code === 'CODEX_QUALIFICATION_FENCE'
        || error?.code === 'CODEX_QUALIFICATION_LEASE_DRIFT'
      ) throw error;
      throw this._qualificationFenceError();
    }
  }
  ...
  _qualificationResult({
    ...
    return Object.freeze({
      outcome,
      reason,
      generationDigest: hash('sha256', this.generationId, 'hex'),
      activityEpoch: this.activityEpoch,
      processState: PROCESS_STATES.has(this.state) ? this.state : 'unknown',
      activeTurnCount: this.current ? 1 : 0,
      pendingDeliveryCount: this.hasPendingDeliveryWork() ? 1 : 0,
      backgroundOwnerCount: this.hasActiveBackgroundWork() ? 1 : 0,
  ...
  async inspectCleanRestartQualification({
    expectedGenerationDigest = hash('sha256', this.generationId, 'hex'),
    expectedActivityEpoch = this.activityEpoch,
    assertLease,
  } = {}) {
    ...
    const generationDigest = hash('sha256', fence.generationId, 'hex');
    if (
      expectedGenerationDigest !== generationDigest
      || expectedActivityEpoch !== fence.activityEpoch
    ) {
      throw this._qualificationFenceError();
    }
    this._assertQualificationFence(fence, assertLease);

    if (this.state !== 'Idle') {
      return this._qualificationResult({
        outcome: 'mismatch',
        reason: this.current ? 'active-turn' : 'process-not-idle',
      });
    }
    if (!this.client || !this.providerSessionId) {
      return this._qualificationResult({
        outcome: 'mismatch',
        reason: 'process-not-ready',
      });
    }

    let backgroundTerminalCount = 0;
    try {
      const page = await this.client.request(
        'thread/backgroundTerminals/list',
        { threadId: this.providerSessionId, limit: 100 },
      ...
      if (
        ...
        throw processError(
          'Codex qualification returned an invalid terminal page',
          'CODEX_QUALIFICATION_FAILED',
        );
      }
      backgroundTerminalCount = page.count;
      this._assertQualificationFence(fence, assertLease);
      const observedAtMs = Date.now();
      if (page.nextCursor != null) {
        return this._qualificationResult({
          outcome: 'mismatch',
          reason: page.count > 0 ? 'background-terminals' : 'pagination-incomplete',
          backgroundTerminalCount,
      ...
      if (this.hasPendingDeliveryWork()) {
        return this._qualificationResult({
          outcome: 'mismatch',
      ...
      throw processError(
        'Codex clean-restart qualification inspection failed',
        'CODEX_QUALIFICATION_FAILED',
      );
    }
  }
  ...
  async _mutation(method, params, detail) {
    ...
    try {
      ...
      const dispositionAware = (
        typeof detail.onPreparationDisposition === 'function'
      );
      const notCommitted = (
        dispositionAware
      ...
      durability.deliveryState = (
        dispositionAware && !notCommitted ? 'ambiguous' : 'not-sent'
      );
      if (dispositionAware) {
        const disposition = {
          kind: notCommitted
        ...
        detail.onDeliveryState?.(disposition.kind);
        detail.onPreparationDisposition(disposition);
      }
      throw durability;
    }
    detail.onDeliveryState?.('prepared');
    detail.onPreparationDisposition?.({ kind: 'checkpoint-committed' });
  ...
  async _checkpoint(kind, detail = {}, fence = null) {
    ...
    try {
      ...
      durability.deliveryState = 'ambiguous';
      if (error?.checkpointDisposition === 'not-committed') {
        durability.checkpointDisposition = 'not-committed';
      }
      throw durability;
    }
    fence?.assertActive?.();
  ...
  async _handleNotificationWithFence(notification, fence) {
    ...
    if (params.threadId && params.threadId !== ownedThreadId) {
      await this._notificationProtocolFault(
        ...
        fence,
        {
          notificationMethod: NOTIFICATION_METHODS.has(method) ? method : null,
          observedProcessState: PROCESS_STATES.has(this.state)
            ? this.state
            : 'unknown',
        },
      );
    ...
    if (method === 'thread/status/changed') {
      ...
      const backgroundCandidate = activateBackground
        ? this._captureBackgroundPromotionCandidate()
        : null;
      if (activateBackground) this._advanceActivityEpoch();
      const backgroundAdmission = activateBackground
        ? Object.freeze({
      ...
      if (activateBackground) {
        try {
          this._promoteBackgroundCandidateToOwner(backgroundAdmission);
        } catch (error) {
          await this._enterContainment(
            'background-ownership-unknown',
            error,
  ...
  async _notificationProtocolFault(reason, message, fence, detail = {}) {
    const error = this._protocolFault(reason, message, false);
    if (this.protocolFaultCount >= this.protocolFaultThreshold) {
      await this._enterContainment(reason, error, null, fence, detail);
    }
    return error;
  }
  ...
  async _settleNaturalBackgroundWork() {
    ...
    try {
      this._assertLifecycle(lifecycle);
      const { owner, terminal } = this._requireBackgroundOwner();
  ...
  _armBackgroundWatchdog() {
    const { owner } = this._requireBackgroundOwner();
    const { deadlineAt } = owner;
    const retirementReserveMs = (
      this.interruptTimeoutMs + this.cleanupTimeoutMs
    );
    if (
    ...
    this.backgroundWatchdog = setTimeout(() => {
      ...
      let currentOwner;
      try {
        currentOwner = this._requireBackgroundOwner().owner;
      } catch (error) {
        this.backgroundWatchdogPromise = this._enterContainment(
          'background-ownership-unknown',
          error,
        ).finally(() => {
          this.backgroundWatchdogPromise = null;
    ...
    }, Math.max(1, deadlineAt - retirementReserveMs - Date.now()));
  ...
  _protocolFault(reason, message, enterContainment = true) {
    const error = attachClientFaultProvenance(
      processError(message, 'CODEX_PROTOCOL_ERROR'),
      {
  ...
  async _enterFailedAmbiguous(reason, error, excludedPending = null) {
    ...
    this.state = 'FailedAmbiguous';
    this.inFlight = false;
    const provenance = readClientFaultProvenance(error);
    try {
  ...
  async _enterContainment(
    ...
    const ownedTurnId = this.activeTurnId
      ...
      ?? this.lastTurnId;
    this._advanceLifecycle();
    const notificationMethod = NOTIFICATION_METHODS.has(detail.notificationMethod)
      ? detail.notificationMethod
      : null;
    this.containmentProvenance = Object.freeze({
      ...readClientFaultProvenance(error),
      ...(notificationMethod ? { notificationMethod } : {}),
      observedProcessState: PROCESS_STATES.has(detail.observedProcessState)
        ? detail.observedProcessState
        : 'unknown',
    });
    this.state = 'ContainmentFailed';
    this.containmentReason = reason;
    this.containmentError = error;
    this.containmentErrorCode = error?.code ?? null;
    this.containmentTurnId = ownedTurnId ?? null;
    this.inFlight = false;
    try {
  ...
  _scheduleContainedTransportClose() {
    if (this.containmentClosePromise) return this.containmentClosePromise;
    this.containmentClosePromise = new Promise((resolve, reject) => {
      setImmediate(() => {
        this._finalizeContainedTransportClose().then(resolve, reject);
      });
    });
    this.containmentClosePromise.catch((closeError) => {
      this.logger.error?.(
        `[${this.label}] Codex containment close: ${closeError.message}`,
      );
    });
    return this.containmentClosePromise;
  }

  async _finalizeContainedTransportClose() {
    await timeout(
    ...
    if (this.state !== 'ContainmentFailed') return null;
    await this._checkpoint('containment-cleanup-completed', {
      reason: this.containmentReason,
      errorCode: this.containmentErrorCode,
      turnId: this.containmentTurnId,
      ...this.containmentProvenance,
    });
    ...
    this.containmentCleanupCommitted = detail;
    this.closed = true;
    this.state = 'Closed';
    this._emitClose(1, null, detail);
    return detail;
  }
  ...
  _drainWaiting(code, reason = 'interrupt') {
    let count = 0;
    for (const pending of [...this.pendingQueue]) {
      if (pending === this.current) continue;
      this._reserveCancellation(pending, code, reason);
      count += 1;
    }
    return count;
  }
  ...
  _drainAll(code, reason = 'interrupt') {
    let count = 0;
    for (const pending of [...this.pendingQueue]) {
      this._reserveCancellation(pending, code, reason);
      count += 1;
  ...
  _reserveCancellation(pending, code, reason) {
    const index = this.pendingQueue.indexOf(pending);
    if (index !== -1) this.pendingQueue.splice(index, 1);
    try {
      pending.cancelled = true;
  ...
  captureCleanRestartCandidate() {
    const pending = this.current;
    const sourceMsgId = pending?.context?.sourceMsgId ?? null;
    const settings = pending?.admittedSettings ?? this.activeTurnSettings;
    if (
      this.state !== 'Active'
      || !pending
      || pending.settled
      ...
      || sourceMsgId == null
      || !settings
      || typeof this.generationId !== 'string'
      || this.generationId.length === 0
      || typeof pending.attemptId !== 'string'
      || pending.attemptId.length === 0
      || typeof this.providerSessionId !== 'string'
      || this.providerSessionId.length === 0
      || typeof this.spawnProfileId !== 'string'
      || this.spawnProfileId.length === 0
    ) return null;
    const seenClientIds = new Set([pending.clientUserMessageId]);
    const seenAttemptIds = new Set([pending.attemptId]);
    for (const follower of this.pendingQueue.slice(1)) {
      const followerSource = follower?.context?.sourceMsgId ?? null;
      if (
        !follower
        || follower === pending
        || follower.settled
        ...
        || follower.startedAt != null
        || follower.startDeliveryState !== 'queued'
        || followerSource == null
        || typeof follower.clientUserMessageId !== 'string'
        || follower.clientUserMessageId.length === 0
        || typeof follower.attemptId !== 'string'
        || follower.attemptId.length === 0
        || seenClientIds.has(follower.clientUserMessageId)
        || seenAttemptIds.has(follower.attemptId)
      ) return null;
      seenClientIds.add(follower.clientUserMessageId);
      seenAttemptIds.add(follower.attemptId);
    }
    return Object.freeze({
      runtime: 'codex',
      namespace: 'codex:app-server',
      ...
      spawnProfileId: this.spawnProfileId,
    });
  }
  ...
  kill(reason = 'kill') {
    ...
    const interruptionReason = boundedOpaqueId(reason, 'interruption reason');
    this.killPromise = this._kill(interruptionReason);
  ...
  async _kill(reason) {
    ...
    if (this.state === 'ContainmentFailed') {
      return this._scheduleContainedTransportClose();
    }
    if (this.state === 'DurabilityBlocked') {
      this._scheduleContainedTransportClose();
      return;
    ...
    this._drainWaiting('KILLED', reason);
    if (!this.current) this._drainAll('KILLED', reason);
    await this._flushPendingCancellations();
    this._assertLifecycle(lifecycle);
  ...
  _emitClose(code, reason = null, exactDetail = null) {
    if (this.closeEmitted) return;
    this.closeEmitted = true;
    this.emit('close', code, exactDetail ?? {
      backend: this.backend,
      generationId: this.generationId,
      reason,
      containmentReason: this.containmentReason,
