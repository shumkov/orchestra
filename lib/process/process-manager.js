...
class ProcessManager {
   ...
   * Codex spawn. A quarantined record is released only by an explicit restore
   * on the same host with a different boot-session identity.
   *
   * @param {{status: 'clear'}|{
   *   status: 'quarantined',
   *   hostIdentity: string,
  ...
  restoreCodexRecoveryState(state) {
    ...
    const sameHost = (
      ...
      && state.hostIdentity === this.codexHostIdentity
    );
    const newBoot = (
      isOpaqueId(this.codexBootSessionIdentity)
      && state.bootSessionIdentity !== this.codexBootSessionIdentity
    );
    this._codexRecoveryReady = true;
    this._codexLease = sameHost && newBoot
      ? null
      : {
  ...
  async _strictRetire(proc, reason) {
    ...
    if (this._isCodex(proc)) {
      ...
      if (this.codexRetirementVerifier) {
        await this._verifyCodexRetirement(proc, reason);
      }
      return;
    }
    await proc.kill(reason);
  ...
  async retireForCleanRestart({ getDeliveryEvidence } = {}) {
    ...
    try {
      settled = await Promise.allSettled(targets.map(async ([sessionKey, proc]) => {
        let snapshot;
        if (this._isCodex(proc)) {
          await getDeliveryEvidence(sessionKey, null);
          await this._strictRetire(proc, 'clean-restart');
          if (this._codexLeaseMatches(proc)) this._codexLease = null;
          snapshot = {
            sourceMsgId: null,
            eligible: false,
        ...
        const cliEligible = proc.backend === 'cli' && snapshot?.eligible === true;
        return Object.freeze({
          sessionKey,
          sourceMsgId: proc.backend === 'cli'
            ? (snapshot?.sourceMsgId ?? null)
            : null,
          eligible: cliEligible,
          reason: proc.backend === 'cli'
