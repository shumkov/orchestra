  ...
  async _spawnTmuxClaude({ tmuxName, opts }) {
    ...
    claudeArgs.push('--append-system-prompt', [
      ...
      ...(this.streamToolEnabled ? [
        '### Showing your answer as you write it',
        '',
        `While you are COMPOSING anything long, call \`mcp__${b}__stream\` with the`,
        'FULL answer-so-far at natural checkpoints (after a paragraph, a section, a',
        'finished thought). Each call replaces the preview, so always send the whole',
        'answer up to that point — never just the new part. The user watches it fill in',
        ...
        '',
        'Skip it for short answers (a couple of sentences) — just reply. And never',
        'retry a stream call: it cannot fail in a way that matters.',
        '',
      ] : []),
