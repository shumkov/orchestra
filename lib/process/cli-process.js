...
class CliProcess extends Process {
  ...
  async _handleBridgeDisconnected(reason = 'socket-close') {
    ...
    this.mcpReady = false;
    if (this.closed) return;
    const disconnectError = await this._bridgeDisconnectError();
  ...
  async _bridgeDisconnectError() {
    ...
    if (this.tmuxSession && typeof this.runner?.sessionExists === 'function') {
      try {
        sessionExists = await this.runner.sessionExists(this.tmuxSession);
      } catch (err) {
        this.logger.warn?.(`[${this.label}] channels: could not verify contained tmux session after bridge disconnect: ${err.message}`);
      }
    }
    if (sessionExists) {
      return Object.assign(new Error('bridge disconnected'), { code: 'BRIDGE_DISCONNECTED' });
