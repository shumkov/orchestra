...
} from './channels-bridge-protocol.js'
import { z } from 'zod'
...
// Live-preview streaming is opt-in per consumer, decided at spawn time by the
// daemon (CliProcess.toolDispatcherCapabilities → _bridgeEnv). A consumer that
// has no live-preview surface never sets it, so the tool is absent from
// ListTools and CallTool rejects it — the agent is never told about a
// capability the other side can't honor. A handshake can't do this job: the
// bridge is the side claude talks to, and ListTools fires once, racing the
// daemon socket.
const STREAM_TOOL_ENABLED = process.env.ORCHESTRA_STREAM_TOOL === '1'
...
function resolveToolAck(msg) {
  const p = pendingToolCalls.get(msg.tool_call_id)
  if (!p) return
...
function handleDaemonMessage(msg) {
  switch (msg.kind) {
    ...
    case 'user_msg':
      ...
      // Remembered so a `stream` call that omits turn_id (a frequent model
      // error — the schema marks it required, models drop it anyway) can be
      // stamped with the turn it is almost certainly about. The daemon still
      // verifies the id against its live head turn, so a stale or foreign
      // stamp is dropped there exactly like any other mismatch — stamping
      // widens nothing, it only rescues the ordinary case.
      if (msg.turn_id) lastUserTurnId = msg.turn_id
      mcp.notification({
...
mcp.setRequestHandler(ListToolsRequestSchema, async () => {
  ...
  tools: [{
    ...
    description: 'Send a message back to the originating Telegram chat. ' +
                 ...
                 'Returns {ok, message_id, attempt_id, delivery, replay_of?}: keep the message_id to update that bubble in place ' +
    ...
    // status primitive. Update one bubble instead of sending several. The edit is
    // re-rendered the same way the reply was, so a checklist stays a checklist.
    name: 'edit_message',
    description: 'Edit a message you previously sent via `reply`, in place. In a chat that ' +
                 'renders rich, an edited checklist/table/heading is re-rendered rich rather ' +
                 'than flattened. Canonical use: send a plan as `- [ ] step` items, then edit ' +
                 'that same message_id with the WHOLE list again and `- [x]` on completed ' +
                 'steps — check items off as you go and the user watches one bubble update, ' +
                 'and one final time when the work ends, every item resolved and nothing ' +
                 'left bold. ' +
                 'Also fine for a short one-line status. An edit REPLACES the body, so always ' +
                 're-send the full text. An edit is TEXT-ONLY: image markdown degrades to its ' +
                 'caption and cannot add media to a bubble (send media with `reply(files:)`). ' +
                 'Keep status in PLAIN LANGUAGE — never tool names like Bash/Edit. One bubble, ' +
                 'never chunked: ~4,000 chars for plain text, much more (~32,000) when it ' +
                 'renders rich; over the limit the tool tells you which one applied — send ' +
                 'long content as a fresh `reply` instead. An edit does NOT notify the user ' +
                 'and carries no consumed_turn_ids: it is a mid-turn update, never the final ' +
                 'answer.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  // Live preview of the answer being composed. Present only when the consumer
  // opted in at spawn time; the matching prompt section is gated on the same
  // flag, so the agent is never coached to call a tool that isn't registered.
  ...(STREAM_TOOL_ENABLED ? [{
    name: 'stream',
    description: 'Show the user the answer you are composing, live, while you are still writing it. ' +
                 'Pass the FULL answer-so-far (cumulative — not a delta) at natural checkpoints ' +
                 'while composing anything long; each call replaces the preview. ' +
                 'This is a preview, NOT a delivery: it never ends a turn, and you MUST still ' +
                 'finish with a normal `reply` carrying the complete answer. ' +
                 'Returns {ok:true} and has no effect on the turn — never retry a failed stream call.',
    inputSchema: {
      type: 'object',
      properties: {
        chat_id: { type: 'string', description: 'Echo of chat_id from inbound channel meta.' },
        turn_id: { type: 'string', description: 'Echo of turn_id from inbound channel meta (required — a snapshot that cannot be attributed to a turn is dropped).' },
        text:    { type: 'string', description: 'The full answer so far (markdown ok).' },
      },
...
const CALLABLE_TOOLS = new Set(
  ['reply', 'ask', 'edit_message', ...(STREAM_TOOL_ENABLED ? ['stream'] : [])],
)

mcp.setRequestHandler(CallToolRequestSchema, async req => {
  if (!CALLABLE_TOOLS.has(req.params.name)) {
  ...
  if (req.params.name === 'stream' && !args?.turn_id && lastUserTurnId) {
    args = { ...args, turn_id: lastUserTurnId }
    log('stream-turn-id-stamped', { turn_id: lastUserTurnId })
  }
  ...
  try {
    ...
    return formatToolAckResult({ ack, toolCallId })
  } catch (err) {
    return formatToolAckResult({ error: err, toolCallId })
  }
})
