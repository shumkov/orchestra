...
function normalizeExpectedProfile(value) {
  ...
  const suppliedManifest = source.hookManifest ?? null;
  const suppliedArtifactsSha256 = source.hookArtifactsSha256 ?? null;
  if ((suppliedManifest === null) !== (suppliedArtifactsSha256 === null)) {
    throw new TypeError(
      'Codex preflight hook manifest and artifact digest must be paired',
    );
  }
  const hookManifest = suppliedManifest === null
    ? null
    : normalizeCodexHookManifest(suppliedManifest);
  const hookArtifactsSha256 = suppliedArtifactsSha256 === null
    ? null
    : requiredSha256(suppliedArtifactsSha256, 'hookArtifactsSha256');
  if (hookManifest !== null && hookManifest.ownedCwd !== cwd) {
    throw new TypeError(
      'Codex preflight hook manifest must own the configured workspace',
    );
  }
  if (
  ...
  const profile = {
    ...
    sessionLauncher,
    sessionLauncherSha256,
    // Absent, not null: a profile prepared without hooks keeps exactly the key
    // set, object model and fingerprint it had before hooks existed.
    ...(hookManifest === null ? {} : { hookManifest, hookArtifactsSha256 }),
  };
...
function projectClientFault(value) {
  ...
  const projected = {
    ...
    clientRootErrorCode: outcome.clientRootErrorCode ?? 'unknown',
    clientFaultClass: outcome.clientFaultClass ?? 'unknown',
  ...
  if (
    ...
    || !CLIENT_ROOT_ERROR_CODES.has(projected.clientRootErrorCode)
    || !CLIENT_FAULT_CLASSES.has(projected.clientFaultClass)
  ) {
    throw new CodexPreflightError(
...
function clientFaultError(outcome) {
  ...
  error.clientRootErrorCode = outcome.clientRootErrorCode;
  error.clientFaultClass = outcome.clientFaultClass;
  return error;
}
...
function staticAttestation(profile, configRead, requirements, profiles) {
  ...
  return {
    ...
    // Hook material joins the identity only when a manifest is carried, so a
    // profile prepared without hooks keeps the fingerprint it always had.
    ...(profile.hookManifest == null ? {} : {
      hookManifestSha256: digest(profile.hookManifest),
      hookArtifactsSha256: profile.hookArtifactsSha256,
    }),
...
async function preflightCodexRuntime(
  ...
  const client = clientFactory({
    ...
    ...(profile.hookManifest == null
      ? {}
      : { hookManifest: profile.hookManifest }),
