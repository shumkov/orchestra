...
async function attestPinnedCodexBinary(binary) {
  ...
  if (sha256 !== protocolSchema.binarySha256) {
    throw new CodexAppServerError(
  ...
  if (stdout.trim() !== protocolSchema.cliVersion) {
    throw new CodexAppServerError(
...
function mutationDeliveryError(pending, cause) {
  ...
  return new CodexAppServerError(message, code, { cause });
}
...
class CodexAppServerClient extends EventEmitter {
  constructor(options = {}) {
    ...
    if (process.platform === 'win32') {
      throw new CodexAppServerError(
        'The native Codex app-server beta does not support Windows',
  ...
  async _start() {
    ...
    const commandArgs = [SUPERVISOR_PATH, this.binary, ...args];
    try {
      const [attestation, homeAttestation] = await Promise.all([
        this.attestBinaryFn(this.binary),
      ...
      if (
        ...
        || attestation.sha256 !== protocolSchema.binarySha256
        || attestation.version !== protocolSchema.cliVersion
      ) {
        throw new CodexAppServerError(
  ...
  _request(method, params, {
    ...
    if (
      (!internal && this.state !== 'ready')
      || (internal && method === 'initialize' && this.state !== 'initializing')
    ) {
      return Promise.reject(new CodexAppServerError(
    ...
    let projectedParams;
    try {
      projectedParams = projectRequestParams(method, params, spec, this.cwd);
    ...
    if (this.usedRequestIds.size >= this.maxUsedRequestIds) {
    ...
    try {
      id = this.requestIdFactory
        ? this.requestIdFactory(this.nextRequestId)
        : this.nextRequestId;
      this.nextRequestId += 1;
      assertRequestId(id);
    ...
    return new Promise((resolve, reject) => {
      ...
      void this._dispatchRequest(pending, projectedParams);
    });
  }
  ...
  async _handleLine(line) {
    ...
    if (hasId) {
      ...
      try {
        if (hasResult) {
          projectedResult = projectRpcResult(
            pending.method,
            message.result,
            this.cwd,
          );
  ...
  _fault(error) {
    ...
    const ambiguousMutation = this._rejectAll(fault);
  ...
  _rejectAll(cause) {
    ...
    for (const pending of this.pending.values()) {
      ...
      const requestError = pending.stateChanging
        ? mutationDeliveryError(pending, cause)
        : cause;
      if (
  ...
  _signalOwnedTree(signal) {
    if (!this._childAlive()) return false;
    if (
  ...
  async _ownedGroupMembers() {
    if (!Number.isSafeInteger(this.child?.pid)) return [];
    let stdout;
    try {
      ({ stdout } = await execFileAsync('/bin/ps', ['-axo', 'pid=,pgid='], {
        encoding: 'utf8',
        timeout: Math.max(100, this.closeGraceMs),
        maxBuffer: 1024 * 1024,
        env: { PATH: CONTROLLED_PATH },
      }));
    } catch (error) {
      throw new CodexAppServerError(
        'could not inspect the owned app-server process group',
  ...
  async _waitForOnlySupervisor(timeoutMs) {
    const deadline = Date.now() + timeoutMs;
    while (this._childAlive() && Date.now() < deadline) {
      const members = await this._ownedGroupMembers();
      if (members.length === 1 && members[0] === this.child.pid) return true;
  ...
  async _close() {
    ...
    if (this._childAlive()) {
      ...
      this._signalOwnedTree('SIGTERM');
      let onlySupervisor = false;
      try {
        onlySupervisor = await this._waitForOnlySupervisor(this.closeGraceMs);
      } catch (error) {
        cleanupError = error;
      }
      if (onlySupervisor && this._childAlive()) {
        try {
          this.killFn(this.child.pid, 'SIGUSR1');
        } catch (error) {
          if (error?.code !== 'ESRCH') cleanupError ??= error;
        }
        await this._waitForExit(this.closeKillMs);
      }
    }
    if (this._childAlive()) {
      this._signalOwnedTree('SIGKILL');
      await this._waitForExit(this.closeKillMs);
    }
    if (this._childAlive()) {
      throw new CodexAppServerError(
        'app-server child survived kill escalation',
